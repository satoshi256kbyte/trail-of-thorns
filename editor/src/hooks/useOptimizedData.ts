...
export const useOptimizedData = ({
  ...
  // Optimized filter functions
  const filterCharactersByStats = useCallback(
  ...
  const filterItemsByRarity = useCallback(
    (rarity: string) => {
  ...
  const validateStageReferences = useCallback(
    (stage: Stage) => {
      const errors: string[] = [];
      ...
      if (stage.enemies) {
        stage.enemies.forEach((enemy, index) => {
          ...
          const characterExists = characters.some(
            char => char.id === enemy.characterId
          );
          if (!characterExists) {
            errors.push(
              `Enemy ${index + 1} references non-existent character: ${enemy.characterId}`
            );
          }
        });
      }
      ...
      if (stage.rewards?.items) {
        stage.rewards.items.forEach((reward, index) => {
          ...
          if (!itemExists) {
            errors.push(
              `Reward item ${index + 1} references non-existent item: ${reward.itemId}`
            );
          }
        });
      }
