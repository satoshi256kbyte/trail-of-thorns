...
class VolumeSlider extends Phaser.GameObjects.Container implements NavigableElement {
  ...
  private setupSlider(labelText: string): void {
    ...
    this.handle = this.scene.add.rectangle((this.value - 0.5) * 200, 0, 20, 20, 0x3498db);
...
export class ConfigScene extends Phaser.Scene {
  ...
  private setupBackground(): void {
    try {
      ...
      this.backgroundGraphics.fillGradientStyle(topColor, topColor, bottomColor, bottomColor, 1);
  ...
  private async handleBack(): Promise<void> {
    try {
      ...
      await SceneTransition.transitionTo(this, 'TitleScene', TransitionType.SLIDE_UP, {
        fromScene: 'ConfigScene',
        action: 'back',
      });
    } catch (error) {
