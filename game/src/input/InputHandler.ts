...
export class InputHandler {
    ...
    private cameraControlCallback?: (direction: 'up' | 'down' | 'left' | 'right', deltaTime: number) => void;
    ...
    constructor(scene: Phaser.Scene, config?: Partial<InputConfig>, eventEmitter?: Phaser.Events.EventEmitter) {
        this.scene = scene;
    ...
    private handlePointerDown(pointer: Phaser.Input.Pointer): void {
        ...
        const isDoubleClick = this.lastClickTime > 0 &&
    ...
    private handleDragStart(pointer: Phaser.Input.Pointer): void {
        ...
        this.eventEmitter?.emit('drag-start', {
            startPosition: this.screenToWorldPosition(this.dragStartPosition!.x, this.dragStartPosition!.y),
    ...
    private validateInput(clickInfo: ClickInfo): InputValidationResult {
        ...
        if (clickInfo.unit) {
            ...
            if (clickInfo.unit.hasActed && clickInfo.type !== 'right' && clickInfo.unit.faction === 'player') {
                return {
                    valid: false,
    ...
    /**
     * Get unit at world position (stub - should be implemented with actual unit detection)
     */
    private getUnitAtPosition(worldPosition: Position): Unit | null {
        // This should be implemented to check for units at the given position
     ...
     */
    setCameraControlCallback(callback: (direction: 'up' | 'down' | 'left' | 'right', deltaTime: number) => void): void {
