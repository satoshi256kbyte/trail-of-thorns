...
export class UIManager {
    ...
    /**
     * Show a general message to the player
     * @param message - Message to display
     * @param type - Message type (success, warning, info, error)
     * @param duration - Display duration in milliseconds
     */
    public showMessage(message: string, type: 'success' | 'warning' | 'info' | 'error' = 'info', duration: number = 3000): void {
     ...
     */
    public showNotification(data: { message: string; type: 'success' | 'warning' | 'info' | 'error'; duration?: number }): void {
        this.showErrorNotification({
            message: data.message,
            type: data.type === 'success' ? 'info' : data.type,
