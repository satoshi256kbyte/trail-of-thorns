...
export interface BattleSystemState {
    ...
    phase: 'idle' | 'range_display' | 'target_selection' | 'battle_execution' | 'animation' | 'cleanup';
...
export class BattleSystem extends Phaser.Events.EventEmitter {
    ...
    public async initiateAttack(attacker: Unit, weapon?: Weapon): Promise<void> {
        ...
        try {
            ...
            this.debugManager.logBattlePhaseEnd(debugTimestamp, undefined, undefined, (error as Error).message);
        ...
        try {
            ...
            if (this.debugManager) {
                const validTargets = this.allUnits
                    ...
                    .filter(unit => rangePositions.some(pos => pos.x === unit.position.x && pos.y === unit.position.y))
    ...
    public async selectTarget(target: Unit, options?: BattleExecutionOptions): Promise<BattleResult> {
        const debugTimestamp = this.debugManager.logBattlePhaseStart('target_selection', this.state.currentAttacker!, target);
    ...
    private async executeBattle(
        ...
        const debugTimestamp = this.debugManager.logBattlePhaseStart('damage_calculation', attacker, target);
        ...
        try {
            ...
            if (!damageContext.isEvaded) {
                ...
                if (battleResult.targetDefeated && this.config.enableAnimations && !options?.skipAnimations) {
            ...
            const experienceResult = this.battleStateManager.grantExperience(attacker, baseExperience, battleResult);
     ...
     */
    private validateAttackInitiation(attacker: Unit, weapon?: Weapon): { success: boolean; error?: BattleError; message?: string } {
        if (!attacker) {
            return {
                success: false,
    ...
    private classifyError(error: Error, context: Partial<BattleContext>): BattleError {
        ...
        if (message.includes('invalid_weapon_type') || message.includes('weapon type is not recognized')) {
