...
export class BattlePerformanceManager {
  ...
  public cleanup(): void {
    ...
    this.performanceMetrics.objectPoolUtilization =
      poolCount > 0 ? totalUtilization / poolCount : 0;
    ...
    if (typeof process !== 'undefined' && process.memoryUsage) {
      ...
      if (this.performanceMetrics.memoryUsage > this.config.memoryWarningThreshold) {
        console.warn(
          `[BattlePerformanceManager] Memory usage warning: ${this.performanceMetrics.memoryUsage.toFixed(2)}MB`
        );
      }
    }
  }
