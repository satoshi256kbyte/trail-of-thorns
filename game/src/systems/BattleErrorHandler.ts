...
import { BattleError, BattleContext, BattleErrorDetails, Weapon } from '../types/battle';
...
export class BattleErrorHandler extends Phaser.Events.EventEmitter {
  ...
  private determineRecoveryStrategy(
    ...
    if ([BattleError.BATTLE_SYSTEM_ERROR, BattleError.DAMAGE_CALCULATION_ERROR].includes(error)) {
      return 'reset';
    }
  ...
  private async provideUserFeedback(
    ...
    this.userFeedback.showErrorMessage(userMessage, messageType, this.config.messageDuration);
  ...
  private isRecoverable(error: BattleError): boolean {
    ...
    return ![BattleError.BATTLE_SYSTEM_ERROR, BattleError.DAMAGE_CALCULATION_ERROR].includes(error);
  }
  ...
  private showErrorMessage(
    ...
    this.errorContainer.setPosition(camera.centerX - this.errorText.width / 2, camera.y + 50);
