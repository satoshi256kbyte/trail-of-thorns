...
export class AIController extends Phaser.Events.EventEmitter {
    ...
    private determineBehaviorType(unit: Unit, targets: AITarget[], allUnits: Unit[]): AIBehaviorType {
        ...
        if (this.hasHealingAbilities(unit)) {
            const injuredAllies = allUnits.filter(u =>
                u.faction === unit.faction &&
                u.currentHP < u.stats.maxHP * 0.5 &&
                u.id !== unit.id
            );
    ...
    private async generatePossibleActions(
        ...
        const movementActions = await this.generateMovementActions(unit, targets, behaviorType, mapData);
    ...
    private async generateMovementActions(
        ...
        switch (behaviorType) {
            ...
            case AIBehaviorType.AGGRESSIVE:
                ...
                for (const target of targets.slice(0, 3)) { // Top 3 targets
                    ...
                    for (const position of closerPositions.slice(0, 2)) { // Top 2 positions
                        actions.push({
            ...
            case AIBehaviorType.DEFENSIVE:
                ...
                const defensivePositions = this.findDefensivePositions(unit, targets, movementRange, mapData);
    ...
    private selectBestAction(actions: AIAction[], behaviorType: AIBehaviorType): AIAction {
        ...
        if (behaviorType === AIBehaviorType.NPC_HUNTER) {
            const npcAttackActions = actions.filter(a =>
                a.type === AIActionType.ATTACK &&
                a.target &&
                this.npcStateManager?.isNPC(a.target)
            );
    ...
    private canReachTarget(unit: Unit, target: Unit, mapData?: MapData): boolean {
        ...
        try {
            const movementRange = this.movementSystem.calculateMovementRange(unit, mapData);
            return movementRange.some(pos =>
                Math.abs(pos.x - target.position.x) <= 1 &&
                Math.abs(pos.y - target.position.y) <= 1
            );
        } catch (error) {
            return false;
        }
    ...
    private findPositionsCloserToTarget(
        ...
        return movementRange
            ...
            .sort((a, b) =>
                this.calculateDistance(a, targetPos) - this.calculateDistance(b, targetPos)
            );
    }
    ...
    private findDefensivePositions(
        ...
        return movementRange
            .map(pos => ({
                position: pos,
                safety: targets.reduce((sum, target) =>
                    sum + this.calculateDistance(pos, target.unit.position), 0
                )
            }))
    ...
    private findTacticalPositions(
        ...
        return movementRange
            .filter(pos => targets.some(t =>
                this.calculateDistance(pos, t.unit.position) <= 3
            ))
            .slice(0, 3);
    }
