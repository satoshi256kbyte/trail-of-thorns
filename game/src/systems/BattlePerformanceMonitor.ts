...
export class BattlePerformanceMonitor {
  ...
  public startOperation(
    operationType: keyof BattlePerformanceMetrics['operationTimes'],
    operationId: string
  ): void {
  ...
  public updateSystemLoad(load: Partial<BattlePerformanceMetrics['systemLoad']>): void {
    ...
    const totalLoad =
      (load.activeUnits || 0) + (load.activeAnimations || 0) + (load.activeEffects || 0);
    if (totalLoad > 50) {
      // Arbitrary threshold
      this.addWarning({
  ...
  public recordBattleCompletion(battleResult: BattleResult, duration: number): void {
    ...
    if (duration > 5000) {
      // Battles taking longer than 5 seconds
      this.addWarning({
        type: PerformanceWarningType.SLOW_OPERATION,
        severity: 'medium',
    ...
    if (
      ...
      if (this.metrics.memory.heapUsed > this.config.memoryWarningThreshold) {
        this.addWarning({
          ...
          severity:
            this.metrics.memory.heapUsed > this.config.memoryWarningThreshold * 1.5
              ? 'high'
              : 'medium',
          ...
          suggestions: ['Enable object pooling', 'Clear unused caches', 'Reduce effect quality'],
        });
      }
    }
  }
  ...
  private analyzePerformance(): void {
    ...
    if (this.metrics.frameRate.average < this.config.frameRateWarningThreshold) {
      this.addWarning({
        ...
        suggestions: ['Reduce animation quality', 'Enable object pooling', 'Optimize calculations'],
      });
    }
   ...
   */
  private getAverageOperationTime(
    operationType: keyof BattlePerformanceMetrics['operationTimes']
  ): number {
  ...
  private getSlowOperationSuggestions(operationType: string): string[] {
    switch (operationType) {
      ...
      case 'stateUpdate':
        return ['Batch state updates', 'Optimize data structures', 'Reduce update frequency'];
      default:
