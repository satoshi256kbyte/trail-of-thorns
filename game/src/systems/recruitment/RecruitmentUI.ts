...
import { RecruitmentErrorHandler, RecruitmentUserFeedbackInterface } from './RecruitmentErrorHandler';
...
export class RecruitmentUI {
    ...
    private createConditionPanel(): void {
        ...
        // Create title
        this.uiElements.conditionTitle = this.scene.add.text(
            this.CONDITION_PANEL_WIDTH / 2,
            20,
            '仲間化条件',
            {
                fontSize: '18px',
        ...
        this.uiElements.progressText = this.scene.add.text(
            this.CONDITION_PANEL_WIDTH / 2,
            55,
            '0%',
            {
    ...
    showNPCIndicator(unit: Unit): void {
        try {
            ...
            // Create "NPC" text
            const npcText = this.scene.add.text(0, 8, 'NPC', {
                fontSize: '10px',
                color: this.COLORS.TEXT_PRIMARY,
                fontFamily: 'Arial',
                fontStyle: 'bold',
    ...
    showRecruitmentSuccess(unit: Unit): void {
        try {
            ...
            // Create success text
            const successTitle = this.scene.add.text(0, -15, '仲間化成功！', {
                fontSize: '18px',
                color: this.COLORS.TEXT_PRIMARY,
                fontFamily: 'Arial',
                fontStyle: 'bold',
            ...
            const successMessage = this.scene.add.text(0, 15, `${unit.name} が仲間になりました`, {
                fontSize: '14px',
                color: this.COLORS.TEXT_PRIMARY,
                fontFamily: 'Arial',
            }).setOrigin(0.5);
            ...
            this.uiElements.successNotification
                .setVisible(true)
                .setScale(0)
                .setAlpha(0);
    ...
    showRecruitmentFailure(unit: Unit, reason: string): void {
        try {
            ...
            // Create failure text
            const failureTitle = this.scene.add.text(0, -15, '仲間化失敗', {
                fontSize: '18px',
                color: this.COLORS.TEXT_PRIMARY,
                fontFamily: 'Arial',
                fontStyle: 'bold',
            ...
            const failureMessage = this.scene.add.text(0, 15, reason, {
                fontSize: '14px',
                color: this.COLORS.TEXT_PRIMARY,
                fontFamily: 'Arial',
            ...
            // Add elements to notification
            this.uiElements.failureNotification.add([
                background,
                xMark,
                failureTitle,
                failureMessage
            ...
            // Show notification with animation
            this.uiElements.failureNotification
                .setVisible(true)
                .setScale(0)
                .setAlpha(0);
    ...
    resize(width: number, height: number): void {
        try {
            ...
            if (this.uiElements.conditionPanel) {
                this.uiElements.conditionPanel.setPosition(
                    20,
                    height - this.CONDITION_PANEL_HEIGHT - 20
                );
            }
     ...
     */
    private drawDashedCircle(graphics: Phaser.GameObjects.Graphics, x: number, y: number, radius: number, color: number, lineWidth: number): void {
     ...
     * @param lineWidth - Line width
     */
    private drawDottedCircle(graphics: Phaser.GameObjects.Graphics, x: number, y: number, radius: number, color: number, lineWidth: number): void {
