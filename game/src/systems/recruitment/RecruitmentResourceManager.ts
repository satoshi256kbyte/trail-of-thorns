...
export class RecruitmentResourceManager {
    ...
    getStatistics(): ResourceStatistics {
        return {
            trackedEventListeners: Array.from(this.tracker.eventListeners.values())
            ...
            potentialLeaks: Array.from(this.leakDetector.referenceCounts.values())
                .filter(count => count > this.leakDetector.leakThreshold).length,
    ...
    private calculateEstimatedMemoryUsage(): number {
        ...
        const listenerCount = Array.from(this.tracker.eventListeners.values())
            .reduce((total, listeners) => total + listeners.length, 0);
