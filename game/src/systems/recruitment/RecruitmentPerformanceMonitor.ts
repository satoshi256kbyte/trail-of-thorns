...
export interface OptimizationAction {
    ...
    type: 'reduce_batch_size' | 'increase_cache_size' | 'defer_operations' | 'cleanup_memory' | 'disable_features';
...
export class RecruitmentPerformanceMonitor {
    ...
    private updateMetrics(): void {
        ...
        const totalSamples = Array.from(this.samples.values())
            .reduce((total, samples) => total + samples.length, 0);
        this.metrics.operationsPerSecond = totalSamples / (this.config.sampleSize * this.config.monitoringInterval / 1000);
    }
    ...
    private performAutoOptimization(): void {
        ...
        for (const action of highPriorityActions.slice(0, 3)) { // Execute top 3
            try {
                action.execute();
    ...
    private startMonitoring(): void {
        const monitor = () => {
            ...
            if (this.lastFrameTime > 0) {
                ...
                // Keep only recent samples
                if (this.frameRates.length > 60) { // 1 second at 60fps
