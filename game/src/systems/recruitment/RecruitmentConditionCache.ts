...
export class RecruitmentConditionCache {
  ...
  updateConfig(newConfig: Partial<CacheConfig>): void {
    ...
    if (
      newConfig.maxMemoryUsage !== undefined &&
      this.getEstimatedMemoryUsage() > newConfig.maxMemoryUsage
    ) {
      this.evictOldestEntries();
    }
  }
  ...
  static analyzePerformance(statistics: CacheStatistics): {
    ...
    if (statistics.hitRatio < 0.5) {
      recommendations.push(
        'Low cache hit ratio - consider increasing TTL or reducing cache invalidation frequency'
      );
      severity = 'medium';
    }

    // Check memory usage
    if (statistics.memoryUsage > 1024 * 1024) {
      // 1MB
    ...
    // Check access time
    if (statistics.averageAccessTime > 1.0) {
      // 1ms
      recommendations.push(
        'Slow cache access - consider optimizing hash function or reducing cache size'
      );
      severity = 'medium';
    }
