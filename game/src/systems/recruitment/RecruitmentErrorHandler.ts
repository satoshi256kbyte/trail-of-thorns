...
export interface RecruitmentUserFeedbackInterface {
    ...
    showRecruitmentErrorMessage(message: string, type: 'error' | 'warning' | 'info', duration?: number): void;
...
export class RecruitmentErrorHandler extends Phaser.Events.EventEmitter {
    ...
    public async handleRecruitmentError(
        ...
        try {
            ...
            // Execute recovery
            const recoveryResult = await this.executeRecovery(recoveryStrategy, errorDetails, currentAttempts);
     ...
     */
    public cleanupRecruitmentState(context: RecruitmentContext): { success: boolean; message: string } {
        try {
     ...
     * @returns Recovery result
     */
    private async executeCancelRecovery(errorDetails: RecruitmentErrorDetails): Promise<RecruitmentErrorRecoveryResult> {
        // Clean up current state
        const cleanupResult = this.cleanupRecruitmentState(errorDetails.context);
     ...
     * @returns Recovery result
     */
    private async executeResetRecovery(errorDetails: RecruitmentErrorDetails): Promise<RecruitmentErrorRecoveryResult> {
        ...
        return {
            ...
            requiresUserAction: true,
            userGuidance: 'The recruitment system has been reset. You can start a new recruitment attempt.',
            stateModified: true,
            recoverable: false
        };
    }
     ...
     * @returns Recovery result
     */
    private async executeIgnoreRecovery(errorDetails: RecruitmentErrorDetails): Promise<RecruitmentErrorRecoveryResult> {
        return {
            success: true,
            action: 'ignore',
     ...
     * @returns Recovery result
     */
    private async executeFallbackRecovery(errorDetails: RecruitmentErrorDetails): Promise<RecruitmentErrorRecoveryResult> {
        // Clean up current state
        const cleanupResult = this.cleanupRecruitmentState(errorDetails.context);
     ...
     * @returns Recovery result
     */
    private async executeCleanupRecovery(errorDetails: RecruitmentErrorDetails): Promise<RecruitmentErrorRecoveryResult> {
        // Clean up current state
        const cleanupResult = this.cleanupRecruitmentState(errorDetails.context);
    ...
    private handleInvalidAttacker(context: RecruitmentContext): RecruitmentErrorRecoveryResult {
        return {
            ...
            requiresUserAction: true,
            userGuidance: 'Use the correct character to perform this recruitment (check recruitment conditions)',
            stateModified: false,
            recoverable: true
        };
    }
    ...
    private async provideUserFeedback(
        ...
        if (recoveryResult.userGuidance && recoveryResult.requiresUserAction && this.config.showRecruitmentHints) {
    ...
    private createUserFeedbackInterface(): RecruitmentUserFeedbackInterface {
        return {
            showRecruitmentErrorMessage: (message: string, type: 'error' | 'warning' | 'info', duration?: number) => {
            ...
            showRecruitmentConfirmDialog: (message: string, onConfirm: () => void, onCancel: () => void) => {
    ...
    private showRecruitmentErrorMessage(message: string, type: 'error' | 'warning' | 'info', duration?: number): void {
    ...
    private showRecruitmentGuidance(message: string, conditions?: string[], position?: Position): void {
        // Implementation would show guidance tooltip
        console.log('[Recruitment Guidance]', message, conditions);
    }

    private showRecruitmentConfirmDialog(message: string, onConfirm: () => void, onCancel: () => void): void {
