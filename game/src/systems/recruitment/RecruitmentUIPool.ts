...
export class RecruitmentUIPool {
  ...
  private getPoolByName(poolName: string): UIElementPool<any> | null {
    switch (poolName) {
      case 'conditionPanel':
        return this.conditionPanelPool;
      case 'progressBar':
        return this.progressBarPool;
      case 'text':
        return this.textPool;
      case 'indicator':
        return this.indicatorPool;
      case 'notification':
  ...
  private getObjectSize(poolName: string): number {
    switch (poolName) {
      case 'conditionPanel':
        return 1024; // Container with graphics
      case 'progressBar':
        return 512; // Graphics object
      case 'text':
        return 256; // Text object
      case 'indicator':
        return 768; // Container with graphics and icon
      case 'notification':
        return 1024; // Container with background
      default:
        return 256;
    }
  }
  ...
  getEfficiencyMetrics(): {
    ...
    const memoryEfficiency =
      this.config.memoryBudget > 0 ? Math.max(0, 1 - totalMemory / this.config.memoryBudget) : 0;
