...
export class CameraController {
    ...
    constructor(scene: Phaser.Scene, mapData?: MapData, config?: Partial<CameraConfig>, eventEmitter?: Phaser.Events.EventEmitter) {
        this.scene = scene;
    ...
    setMapBounds(mapData: MapData): GameplayErrorResult {
        try {
            if (!mapData || typeof mapData.width !== 'number' || typeof mapData.height !== 'number' || typeof mapData.tileSize !== 'number' ||
                mapData.width <= 0 || mapData.height <= 0 || mapData.tileSize <= 0) {
                return {
                    success: false,
     ...
     * @returns GameplayErrorResult indicating success or failure
     */
    moveCamera(direction: 'up' | 'down' | 'left' | 'right', deltaTime: number = 16): GameplayErrorResult {
        try {
    ...
    setZoom(zoomLevel: number, duration?: number): GameplayErrorResult {
        try {
            ...
            const zoomDuration = duration ?? (Math.abs(clampedZoom - this.camera.zoom) / this.config.zoomSpeed * 1000);
