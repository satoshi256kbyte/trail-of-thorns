...
export class EquipmentManager {
  ...
  public getEffectiveWeapon(unit: Unit): Weapon | null {
    ...
    effectiveWeapon.attackPower = Math.floor(
      effectiveWeapon.attackPower * durabilityStatus.performanceMultiplier
    );
    effectiveWeapon.accuracy = Math.floor(
      effectiveWeapon.accuracy * durabilityStatus.performanceMultiplier
    );
   ...
   */
  public damageEquipmentDurability(
    unit: Unit,
    slot: keyof CharacterEquipment,
    damage: number = 1
  ): Unit {
  ...
  public getEquipmentSummary(unit: Unit): string {
    ...
    if (unit.equipment.weapon) {
      ...
      if (weaponData) {
        const durabilityStatus = this.weaponDataLoader.getWeaponDurabilityStatus(weaponData);
        lines.push(
          `Weapon: ${weaponData.name} (${durabilityStatus.status}, ${Math.round(durabilityStatus.percentage)}%)`
        );
      } else {
    ...
    // Armor
    if (unit.equipment.armor) {
      const durabilityPercentage = Math.round(
        (unit.equipment.armor.durability / unit.equipment.armor.maxDurability) * 100
      );
