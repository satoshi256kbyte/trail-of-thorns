...
export class BattleAnimator extends Phaser.Events.EventEmitter {
    ...
    public async playDefeatedAnimation(unit: Unit): Promise<void> {
        ...
        return new Promise((resolve) => {
            ...
            if (this.config.enableParticleEffects) {
                this.createParticleEffect(unit.sprite.x, unit.sprite.y, 0xff0000, DamageType.PHYSICAL, true);
            }
     ...
     */
    private createParticleEffect(x: number, y: number, color: number, type: DamageType, burst: boolean = false): void {
