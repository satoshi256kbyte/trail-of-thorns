...
export class DamageCalculator {
    ...
    public calculateBaseDamage(attacker: Unit, target: Unit, weapon: Weapon): number {
        ...
        switch (this.config.baseDamageFormula) {
            ...
            case 'square_root':
                ...
                baseDamage = Math.sqrt(effectiveAttack * effectiveAttack - effectiveDefense * effectiveDefense * this.config.defenseEffectiveness);
                break;
     ...
     */
    public applyElementalModifier(damage: number, attackElement: Element, targetElement: Element): number {
    ...
    public performCompleteCalculation(
        ...
        calculationSteps.push(`Base damage: ${baseDamage} (Attack: ${weapon.attackPower + attacker.stats.attack}, Defense: ${target.stats.defense})`);

        // Step 2: Check for evasion
        const evasionResult = this.calculateEvasion(attacker, target, weapon);
        calculationSteps.push(`Evasion check: ${evasionResult.isEvaded ? 'EVADED' : 'HIT'} (${evasionResult.chance}% chance, rolled ${evasionResult.roll})`);
        ...
        calculationSteps.push(`Critical check: ${criticalResult.isCritical ? 'CRITICAL' : 'NORMAL'} (${criticalResult.chance}% chance, rolled ${criticalResult.roll})`);
        ...
        if (elementalDamage !== baseDamage) {
            ...
            calculationSteps.push(`Elemental modifier: ${elementalMultiplier}x (${weapon.element} element)`);
        }

        // Step 5: Apply equipment modifiers
        const equipmentModifiers = this.equipmentManager.getEquipmentDamageModifiers(attacker, target, weapon.element);
        modifiers.push(...equipmentModifiers);

        equipmentModifiers.forEach(modifier => {
            calculationSteps.push(`Equipment modifier: ${modifier.description} (${modifier.multiplier}x)`);
        });
     ...
     */
    public getDamagePreview(attacker: Unit, target: Unit, weapon: Weapon): { min: number; max: number; average: number } {
