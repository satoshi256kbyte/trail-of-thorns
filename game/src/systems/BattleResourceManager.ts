...
export class BattleResourceManager {
    ...
    private eventListeners: Map<string, { target: any; event: string; callback: Function }> = new Map();
    ...
    public registerEventListener(
        ...
        const id = this.registerResource({ target, event, callback }, ResourceType.EVENT_LISTENERS, metadata);
    ...
    private cleanupResource(entry: ResourceEntry): void {
        try {
            ...
            console.error(`[BattleResourceManager] Error cleaning up ${entry.type} resource ${entry.id}:`, error);
        }
    }
    ...
    private updateStatistics(): void {
        ...
        if (this.statistics.totalResources > this.config.resourceWarningThreshold) {
            console.warn(`[BattleResourceManager] High resource count: ${this.statistics.totalResources}`);
        }
    }
    ...
    public getHealthReport(): {
        ...
        if (leakResult.severity === 'critical' || stats.totalResources > this.config.resourceWarningThreshold * 2) {
            status = 'critical';
        } else if (leakResult.severity === 'high' || stats.totalResources > this.config.resourceWarningThreshold) {
            status = 'warning';
        }
        ...
        if (stats.oldestResource > 600000) { // 10 minutes
