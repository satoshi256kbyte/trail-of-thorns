...
export class RecruitmentDataManager {
  ...
  getAvailableRecruitedCharacters(chapterId: string): RecruitedCharacterSaveData[] {
    ...
    return this.currentSaveData.recruitedCharacters.filter(
      character =>
  ...
  private sanitizeRecruitedCharacters(
    characters: RecruitedCharacterSaveData[]
  ...
  private sanitizeChapterProgress(
    progress: Record<string, ChapterProgressData>
  ): Record<string, ChapterProgressData> {
  ...
  private async migrateDataIfNeeded(
    data: RecruitmentSaveData
  ): Promise<{ migrated: boolean; data?: RecruitmentSaveData }> {
    const currentVersion = RecruitmentDataManager.CURRENT_VERSION;

    // Check if migration is needed
    if (
      data.version.major === currentVersion.major &&
