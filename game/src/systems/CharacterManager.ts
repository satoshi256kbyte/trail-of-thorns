...
export class CharacterManager {
    ...
    private createCharacterSprite(unit: Unit): GameplayErrorResult {
        try {
            ...
            const sprite = this.scene.add.sprite(worldPos.x, worldPos.y, textureKey) as CharacterSprite;
    ...
    /**
     * Update character position without marking as moved (for external movement systems)
     *
     * @param characterId ID of character to update
     * @param newPosition New grid position
     * @returns GameplayErrorResult indicating success or failure
     */
    updateCharacterPosition(characterId: string, newPosition: Position): GameplayErrorResult {
        try {
            // Validate inputs
            if (!TypeValidators.isValidPosition(newPosition)) {
                return {
            ...
            // Update sprite position immediately (no animation for external updates)
            const sprite = this.characterSprites.get(characterId);
            if (sprite) {
                const worldPos = this.gridToWorldPosition(newPosition);
                sprite.setPosition(worldPos.x, worldPos.y);
            ...
            this.eventEmitter?.emit('character-position-updated', {
                character: character,
                oldPosition: oldPosition,
                newPosition: newPosition,
            });
    ...
    /**
     * Update character visual state (e.g., for NPC status)
     * @param characterId - Character ID to update
     * @param state - Visual state to apply
     */
    updateCharacterVisualState(characterId: string, state: 'normal' | 'npc' | 'selected' | 'disabled'): void {
        const sprite = this.characterSprites.get(characterId);
        if (!sprite) {
            console.warn(`Character sprite not found for ID: ${characterId}`);
            return;
        }
        ...
        switch (state) {
            case 'npc':
                // Apply blue tint for NPC state
                sprite.setTint(0x4444ff);
                break;
            case 'selected':
                // Apply yellow tint for selected state
                sprite.setTint(0xffff44);
                break;
            case 'disabled':
                // Apply gray tint and reduce alpha for disabled state
                sprite.setTint(0x888888);
                sprite.setAlpha(0.6);
                break;
            case 'normal':
            default:
                // Normal state - no special effects
                break;
        }
    }

    /**
     * Update character faction (for recruited units)
     * @param characterId - Character ID to update
     * @param faction - New faction
     */
    updateCharacterFaction(characterId: string, faction: 'player' | 'enemy'): void {
        const sprite = this.characterSprites.get(characterId);
        if (!sprite) {
            console.warn(`Character sprite not found for ID: ${characterId}`);
            return;
        ...
        // Update visual representation based on faction
        // This could involve changing sprite color, adding faction indicators, etc.
        if (faction === 'player') {
            // Apply subtle green tint for player units
            sprite.setTint(0x44ff44);
        } else {
            // Apply subtle red tint for enemy units
            sprite.setTint(0xff4444);
        }

        console.log(`Updated character ${characterId} faction to ${faction}`);
    }
