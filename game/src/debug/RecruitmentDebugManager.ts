...
export class RecruitmentDebugManager {
  ...
  public endDebugSession(): RecruitmentDebugSession | null {
    ...
    if (this.currentSession.conditionResults.length > 0) {
      ...
      this.currentSession.averageEvaluationTime =
        totalTime / this.currentSession.conditionResults.length;
    }
  ...
  public logRecruitmentAttempt(
    ...
    const conditionsMetPercentage =
      result.conditionsMet.length > 0
        ? (conditionsMetCount / result.conditionsMet.length) * 100
        : 0;

    this.statistics.averageConditionsMet =
      (this.statistics.averageConditionsMet * (this.statistics.totalAttempts - 1) +
        conditionsMetPercentage) /
      this.statistics.totalAttempts;
    ...
    const report = [
      ...
      `Success Rate: ${
        this.statistics.totalAttempts > 0
          ? (
              (this.statistics.successfulRecruitments / this.statistics.totalAttempts) *
              100
            ).toFixed(2)
          : 0
      }%`,
