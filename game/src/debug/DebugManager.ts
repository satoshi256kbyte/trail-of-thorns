...
export class DebugManager {
    ...
    private handleBattleDebugCommand(action: string, ...args: any[]): void {
        ...
        switch (action.toLowerCase()) {
            case 'state':
                console.log('Battle System State:', this.battleSystem.getCurrentState?.() || 'No state available');
                break;
            ...
            case 'attack':
                if (args.length >= 2) {
                    ...
                    if (attacker && target) {
                        this.battleSystem.initiateAttack?.(attacker)
            ...
            case 'simulate':
                if (this.battleDebugManager && args.length > 0) {
                    const scenarioName = args[0];
                    this.battleDebugManager.runSimulationScenario(scenarioName)
                        .then((results: any) => {
            ...
            case 'config':
                if (args.length > 0) {
                    ...
                    console.log('Available config keys: globalDamageMultiplier, criticalDamageMultiplier, animationSpeed');
                }
                break;
            ...
            default:
                ...
                console.log('  battle attack <attacker_id> <target_id> - Execute battle between characters');
