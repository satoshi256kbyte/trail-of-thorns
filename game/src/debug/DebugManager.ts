...
export class DebugManager {
    ...
    public executeConsoleCommand(commandLine: string): void {
        ...
        if (command) {
            ...
            console.warn(`DebugManager: Unknown command '${commandName}'. Type 'help' for available commands.`);
        }
    }
    ...
    private showGridCoordinates(): void {
        ...
        for (let x = Math.max(0, startX); x < endX; x++) {
            for (let y = Math.max(0, startY); y < endY; y++) {
                ...
                const coordText = this.scene.add.text(worldX, worldY, `${x},${y}`, {
                    fontSize: '10px',
    ...
    private updateGridCoordinates(): void {
        ...
        if (Math.abs(camera.scrollX - lastCameraPos.x) > 32 ||
    ...
    private createCharacterStatDisplay(character: Unit): Phaser.GameObjects.Container {
        ...
        const statsDisplay = this.formatCharacterStats(character);
        const statText = this.scene.add.text(0, 0, statsDisplay, {
            fontSize: '8px',
            color: this.config.textColor,
            fontFamily: 'monospace',
            align: 'center'
    ...
    private updatePerformanceMetrics(time: number, delta: number): void {
        ...
        if (time - this.lastPerformanceUpdate >= this.config.performanceUpdateInterval) {
            this.performanceMetrics.fps = Math.round(this.frameCount * 1000 / (time - this.lastPerformanceUpdate));
    ...
    private setupKeyboardShortcuts(): void {
        ...
        this.scene.input.keyboard?.addKey('BACKTICK').on('down', () => {
            if (this.isEnabled) {
                ...
                console.log('Available commands: help, grid, stats, perf, camera, gamestate, character, clear');
            }
        });
    }
}