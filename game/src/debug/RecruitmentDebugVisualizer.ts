...
export class RecruitmentDebugVisualizer {
  ...
  public visualizeRecruitableCharacter(
    ...
    const conditionTexts = character.conditions.map(
      (condition, index) => `${index + 1}. ${condition.description}`
    );
  ...
  public visualizeConditionEvaluation(
    ...
    const symbol = this.scene.add
      .text(screenPosition.x - 30, screenPosition.y - 20, result ? '✓' : '✗', {
        fontSize: '14px',
        color: '#ffffff',
        align: 'center',
      })
    ...
    if (overlayInfo) {
      const conditionIndex = overlayInfo.conditions.findIndex(
        desc => desc === condition.description
      );
      if (conditionIndex >= 0) {
   ...
   * Visualize NPC state
   */
  public visualizeNPCState(unit: Unit, npcState: NPCState, screenPosition: Position): void {
    if (!this.debugEnabled) return;
  ...
  public visualizeRecruitmentProgress(
    ...
    const progressText = this.scene.add
      .text(screenPosition.x, screenPosition.y + 45, `${progress.overallProgress}%`, {
        fontSize: '10px',
        color: '#ffffff',
        align: 'center',
      })
   ...
   */
  public visualizeRecruitmentSuccess(unit: Unit, screenPosition: Position): void {
    if (!this.debugEnabled) return;
    ...
    const successText = this.scene.add
      .text(screenPosition.x, screenPosition.y - 60, `${unit.name}\nRECRUITED!`, {
        fontSize: '16px',
        color: '#00ff00',
        align: 'center',
        stroke: '#000000',
   ...
   */
  public visualizeRecruitmentFailure(unit: Unit, reason: string, screenPosition: Position): void {
    if (!this.debugEnabled) return;
  ...
  public showDebugOverlay(): void {
    ...
    for (const [characterId, info] of this.debugOverlays) {
      ...
      if (info.npcState) {
        overlayInfo.push(
          `  NPC State: Turn ${info.npcState.convertedAt}, HP ${info.npcState.remainingHP}`
        );
      }
    ...
    const overlayText = this.scene.add.text(10, 10, overlayInfo.join('\n'), {
      fontSize: '12px',
      color: '#ffffff',
  ...
  public getDebugStatistics(): any {
    return {
      ...
      visualizationTypes: Array.from(this.visualElements.values()).reduce(
        (types, element) => {
