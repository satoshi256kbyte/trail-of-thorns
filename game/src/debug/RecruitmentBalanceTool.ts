...
export class RecruitmentBalanceTool {
  ...
  public async runBalanceTest(
    ...
    for (let i = 0; i < config.iterations; i++) {
      ...
      const hpThreshold =
        config.hpThresholds[Math.floor(Math.random() * config.hpThresholds.length)];
    ...
    const averageConditionsMet =
      (totalConditionsMet / (config.iterations * character.conditions.length)) * 100;
    ...
    const isBalanced =
      successRate >= config.targetSuccessRate.min && successRate <= config.targetSuccessRate.max;
  ...
  public async runComprehensiveAnalysis(
    ...
    const report: BalanceAnalysisReport = {
      ...
      overallSuccessRate:
        characterResults.length > 0 ? totalSuccessRate / characterResults.length : 0,
   ...
   */
  public getConditionEffectivenessAnalysis(): Map<
    RecruitmentConditionType,
  ...
  public exportResults(): string {
    const exportData = {
      ...
      testConfigurations: Array.from(this.testConfigurations.entries()).map(([name, config]) => ({
        name,
        ...config,
      })),
    };
  ...
  private generateRecommendations(
    ...
    if (successRate < config.targetSuccessRate.min) {
      recommendations.push(
        `Success rate too low (${successRate.toFixed(2)}%). Consider relaxing conditions.`
      );
      ...
      for (const [conditionId, successCount] of Object.entries(conditionBreakdown)) {
        ...
        if (conditionSuccessRate < 30) {
          recommendations.push(
            `Condition '${conditionId}' is too restrictive (${conditionSuccessRate.toFixed(2)}% success rate).`
          );
        }
      }
    } else if (successRate > config.targetSuccessRate.max) {
      recommendations.push(
        `Success rate too high (${successRate.toFixed(2)}%). Consider tightening conditions.`
      );
    ...
    for (const [conditionId, successCount] of Object.entries(conditionBreakdown)) {
      ...
      if (conditionSuccessRate > 90) {
        recommendations.push(
          `Condition '${conditionId}' is almost always met. Consider making it more challenging.`
        );
      } else if (conditionSuccessRate < 10) {
        recommendations.push(
          `Condition '${conditionId}' is rarely met. Consider making it more achievable.`
        );
      }
    }
    ...
    if (conditionCount > 4) {
      recommendations.push(
        'Character has many conditions. Consider simplifying for better player experience.'
      );
    } else if (conditionCount === 1) {
      recommendations.push(
        'Character has only one condition. Consider adding variety with additional conditions.'
      );
    }
  ...
  private generateGlobalRecommendations(results: BalanceTestResult[]): string[] {
    ...
    if (balancedCount / results.length < 0.7) {
      recommendations.push(
        'Less than 70% of characters are well-balanced. Review global balance settings.'
      );
    }
    ...
    if (performanceIssues > 0) {
      recommendations.push(
        `${performanceIssues} characters have performance issues. Optimize condition evaluation.`
      );
    }
    ...
    if (averageSuccessRate < 20) {
      recommendations.push(
        'Overall success rates are too low. Consider global difficulty reduction.'
      );
    } else if (averageSuccessRate > 70) {
      recommendations.push(
        'Overall success rates are too high. Consider global difficulty increase.'
      );
    }
    ...
    for (const [type, data] of conditionAnalysis) {
      if (data.totalTests > 10) {
        if (data.effectivenessScore < 20) {
          recommendations.push(
            `${type} conditions are too restrictive (${data.effectivenessScore.toFixed(2)}% effectiveness).`
          );
        } else if (data.effectivenessScore > 80) {
          recommendations.push(
            `${type} conditions are too lenient (${data.effectivenessScore.toFixed(2)}% effectiveness).`
          );
        }
      }
    }
  ...
  private generateConfigurationSuggestions(results: BalanceTestResult[]): Record<string, any> {
    ...
    const averageEvaluationTime =
      results.reduce((sum, r) => sum + r.averageEvaluationTime, 0) / results.length;
  ...
  private logAnalysisReport(report: BalanceAnalysisReport): void {
    ...
    console.log(
      `Balanced Characters: ${report.balancedCharacters}/${report.totalCharactersTested}`
    );
