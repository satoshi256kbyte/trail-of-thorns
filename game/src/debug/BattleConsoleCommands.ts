...
export class BattleConsoleCommands {
  ...
  private initializeConsoleCommands(): void {
    ...
    (window as any).battleCommands = {
      ...
      simulateBattle: (attackerId: string, targetId: string, count?: number) =>
        this.simulateBattle(attackerId, targetId, count),
      testDamageCalculation: (attackerId: string, targetId: string) =>
        this.testDamageCalculation(attackerId, targetId),
    ...
    console.log(
      '戦闘コンソールコマンドが利用可能です。battleCommands.help() でヘルプを表示できます。'
    );
  }
  ...
  private testBattle(attackerId: string, targetId: string): CommandResult {
    ...
    const finalDamage = isEvaded
      ? 0
      : criticalResult.isCritical
        ? baseDamage * criticalResult.multiplier
        : baseDamage;
