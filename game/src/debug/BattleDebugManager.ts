...
export interface BattleDebugInfo {
    timestamp: number;
    phase: 'range_calculation' | 'target_selection' | 'damage_calculation' | 'animation' | 'state_update';
    attacker: {
        id: string;
        name: string;
        position: Position;
...
export class BattleDebugManager {
    ...
    private updateScreenDisplay(): void {
        ...
        recentBattles.forEach((info, index) => {
            ...
            if (info.result) {
                const result = info.result.isEvaded ? 'MISS' :
                    info.result.isCritical ? `CRIT ${info.result.damage}` :
        ...
        if (this.displayOptions.showPerformanceMetrics) {
            ...
            const perfText = this.scene.add.text(0, yOffset,
        ...
        if (this.displayOptions.showBattleStatistics) {
            ...
            const statsText = this.scene.add.text(0, yOffset,
