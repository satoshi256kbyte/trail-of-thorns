...
export class GameplayErrorHandler {
     ...
     */
    private handleInvalidStageData(result: GameplayErrorResult, context: ErrorContext): GameplayErrorResult {
     ...
     */
    private handleCharacterLoadFailed(result: GameplayErrorResult, context: ErrorContext): GameplayErrorResult {
        if (this.config.showUserNotifications) {
            this.showUserNotification(
     ...
     */
    private handleMapLoadFailed(result: GameplayErrorResult, context: ErrorContext): GameplayErrorResult {
        if (this.config.showUserNotifications) {
            this.showUserNotification(
                'Map Loading Error',
     ...
     */
    private handleInvalidAction(result: GameplayErrorResult, context: ErrorContext): GameplayErrorResult {
        if (this.config.showUserNotifications) {
            this.showUserNotification(
                'Invalid Action',
     ...
     */
    private handleCameraBoundsError(result: GameplayErrorResult, context: ErrorContext): GameplayErrorResult {
     ...
     */
    private handleInvalidPosition(result: GameplayErrorResult, context: ErrorContext): GameplayErrorResult {
        if (this.config.showUserNotifications) {
            this.showUserNotification(
                'Invalid Position',
     ...
     */
    private handleUnitNotFound(result: GameplayErrorResult, context: ErrorContext): GameplayErrorResult {
        if (this.config.logToConsole) {
     ...
     */
    private handleInvalidTurnState(result: GameplayErrorResult, context: ErrorContext): GameplayErrorResult {
        if (this.config.showUserNotifications) {
            this.showUserNotification(
                'Game State Error',
     ...
     */
    private handleGenericError(result: GameplayErrorResult, context: ErrorContext): GameplayErrorResult {
        if (this.config.showUserNotifications) {
            this.showUserNotification(
                'Unexpected Error',
     ...
     */
    private showUserNotification(title: string, message: string, type: 'info' | 'warning' | 'error'): void {
    ...
    async executeRecoveryAction(recoveryAction: RecoveryAction): Promise<boolean> {
        ...
        while (attempts < this.config.maxRetryAttempts) {
            try {
                ...
                if (success) {
                    console.log(`Recovery action "${recoveryAction.description}" succeeded on attempt ${attempts + 1}`);
                    return true;
                }
            } catch (error) {
                console.error(`Recovery action "${recoveryAction.description}" failed on attempt ${attempts + 1}:`, error);
            }
        ...
        console.error(`Recovery action "${recoveryAction.description}" failed after ${this.config.maxRetryAttempts} attempts`);
        return false;
    }
}
