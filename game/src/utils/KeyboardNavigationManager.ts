...
export class KeyboardNavigationManager {
    ...
    if (index !== -1) {
      ...
      if (index === this.currentFocusIndex) {
        ...
        if (this.navigableElements.length > 0) {
          const nextIndex =
            index < this.navigableElements.length ? index : this.navigableElements.length - 1;
  ...
  private updateFocusIndicator(element: NavigableElement): void {
    ...
    const bounds = (displayObject as any).getBounds
      ? (displayObject as any).getBounds()
  ...
  private activateCurrentElement(): void {
    if (
      ...
      this.currentFocusIndex >= this.navigableElements.length
    ) {
      return;
    }
