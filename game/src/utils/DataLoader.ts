...
export class DataLoader {
    ...
    private static validateStageData(data: any): void {
        ...
        data.stages.forEach((stage: any, index: number) => {
            ...
            if (!stage || typeof stage !== 'object') {
                throw new DataValidationError(`Invalid stage at index ${index}: must be an object`, stagePrefix);
            }

            // Required fields
            if (!stage.id || typeof stage.id !== 'string') {
                throw new DataValidationError(`Invalid stage ID at index ${index}: must be a non-empty string`, `${stagePrefix}.id`);
            }

            if (!stage.name || typeof stage.name !== 'string') {
                throw new DataValidationError(`Invalid stage name at index ${index}: must be a non-empty string`, `${stagePrefix}.name`);
            }

            if (!stage.description || typeof stage.description !== 'string') {
                throw new DataValidationError(`Invalid stage description at index ${index}: must be a non-empty string`, `${stagePrefix}.description`);
            }

            if (typeof stage.isUnlocked !== 'boolean') {
                throw new DataValidationError(`Invalid stage isUnlocked at index ${index}: must be a boolean`, `${stagePrefix}.isUnlocked`);
            }

            if (typeof stage.difficulty !== 'number' || stage.difficulty < 1 || stage.difficulty > 10) {
                throw new DataValidationError(`Invalid stage difficulty at index ${index}: must be a number between 1 and 10`, `${stagePrefix}.difficulty`);
            }

            if (typeof stage.order !== 'number' || stage.order < 1) {
                throw new DataValidationError(`Invalid stage order at index ${index}: must be a positive number`, `${stagePrefix}.order`);
            }

            // Optional fields
            if (stage.thumbnail && typeof stage.thumbnail !== 'string') {
                throw new DataValidationError(`Invalid stage thumbnail at index ${index}: must be a string`, `${stagePrefix}.thumbnail`);
            }
        });
        ...
        if (duplicateIds.length > 0) {
            throw new DataValidationError(`Duplicate stage IDs found: ${duplicateIds.join(', ')}`, 'stages');
        }
        ...
        if (duplicateOrders.length > 0) {
            throw new DataValidationError(`Duplicate stage orders found: ${duplicateOrders.join(', ')}`, 'stages');
        }
    }
    ...
    private static validateConfigData(data: any): void {
        ...
        if (!data.defaultConfig || typeof data.defaultConfig !== 'object') {
            throw new DataValidationError('Invalid config data: defaultConfig must be an object', 'defaultConfig');
        }
        ...
        if (!config.audio || typeof config.audio !== 'object') {
            throw new DataValidationError('Invalid audio config: must be an object', 'defaultConfig.audio');
        }
        ...
        audioFields.forEach(field => {
            ...
            if (typeof value !== 'number' || value < 0 || value > 1) {
                throw new DataValidationError(`Invalid ${field}: must be a number between 0 and 1`, `defaultConfig.audio.${field}`);
            }
        });

        // Validate graphics settings
        if (!config.graphics || typeof config.graphics !== 'object') {
            throw new DataValidationError('Invalid graphics config: must be an object', 'defaultConfig.graphics');
        }

        if (typeof config.graphics.fullscreen !== 'boolean') {
            throw new DataValidationError('Invalid fullscreen setting: must be a boolean', 'defaultConfig.graphics.fullscreen');
        }

        if (!config.graphics.resolution || typeof config.graphics.resolution !== 'string') {
            throw new DataValidationError('Invalid resolution setting: must be a non-empty string', 'defaultConfig.graphics.resolution');
        }

        const validQualities = ['low', 'medium', 'high', 'ultra'];
        if (!validQualities.includes(config.graphics.quality)) {
            throw new DataValidationError(`Invalid quality setting: must be one of ${validQualities.join(', ')}`, 'defaultConfig.graphics.quality');
        }

        // Validate controls settings
        if (!config.controls || typeof config.controls !== 'object') {
            throw new DataValidationError('Invalid controls config: must be an object', 'defaultConfig.controls');
        }

        if (!config.controls.keyBindings || typeof config.controls.keyBindings !== 'object') {
            throw new DataValidationError('Invalid keyBindings: must be an object', 'defaultConfig.controls.keyBindings');
        }

        const requiredKeys = ['up', 'down', 'left', 'right', 'action', 'menu'];
        requiredKeys.forEach(key => {
            if (!config.controls.keyBindings[key] || typeof config.controls.keyBindings[key] !== 'string') {
                throw new DataValidationError(`Invalid key binding for ${key}: must be a non-empty string`, `defaultConfig.controls.keyBindings.${key}`);
            }
        });

        // Validate gameplay settings
        if (!config.gameplay || typeof config.gameplay !== 'object') {
            throw new DataValidationError('Invalid gameplay config: must be an object', 'defaultConfig.gameplay');
        }

        const validDifficulties = ['easy', 'normal', 'hard', 'expert'];
        if (!validDifficulties.includes(config.gameplay.difficulty)) {
            throw new DataValidationError(`Invalid difficulty setting: must be one of ${validDifficulties.join(', ')}`, 'defaultConfig.gameplay.difficulty');
        }

        if (typeof config.gameplay.autoSave !== 'boolean') {
            throw new DataValidationError('Invalid autoSave setting: must be a boolean', 'defaultConfig.gameplay.autoSave');
        }

        if (typeof config.gameplay.showTutorials !== 'boolean') {
            throw new DataValidationError('Invalid showTutorials setting: must be a boolean', 'defaultConfig.gameplay.showTutorials');
        }
    }
