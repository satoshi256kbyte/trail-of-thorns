...
export class WeaponDataLoader {
   ...
   */
  public async loadEquipmentData(): Promise<
    DataLoadResult<{ armor: Equipment[]; accessories: Equipment[] }>
  > {
    try {
      ...
      if (
        !rawData.armor ||
        !Array.isArray(rawData.armor) ||
        !rawData.accessories ||
        !Array.isArray(rawData.accessories)
      ) {
        return {
          success: false,
      ...
      for (const rawAccessory of rawData.accessories) {
        ...
        if (validationResult.success && validationResult.data) {
          ...
          validationErrors.push(
            `Accessory ${rawAccessory.id || 'unknown'}: ${validationResult.error}`
          );
        }
      }
  ...
  private validateAndConvertWeapon(rawWeapon: RawWeaponData): DataLoadResult<Weapon> {
    try {
      ...
      if (
        typeof rawWeapon.criticalRate !== 'number' ||
        rawWeapon.criticalRate < 0 ||
        rawWeapon.criticalRate > 100
      ) {
        return { success: false, error: 'Invalid critical rate (must be 0-100)' };
      }

      if (
        typeof rawWeapon.accuracy !== 'number' ||
        rawWeapon.accuracy < 0 ||
        rawWeapon.accuracy > 100
      ) {
  ...
  private validateAndConvertEquipment(rawEquipment: RawEquipmentData): DataLoadResult<Equipment> {
    try {
      ...
      if (
        rawEquipment.attackBonus !== undefined &&
        (typeof rawEquipment.attackBonus !== 'number' || rawEquipment.attackBonus < 0)
      ) {
        return { success: false, error: 'Invalid attack bonus' };
      }

      if (
        rawEquipment.defenseBonus !== undefined &&
        (typeof rawEquipment.defenseBonus !== 'number' || rawEquipment.defenseBonus < 0)
      ) {
      ...
      if (
        rawEquipment.movementBonus !== undefined &&
        typeof rawEquipment.movementBonus !== 'number'
      ) {
      ...
      if (
        typeof rawEquipment.maxDurability !== 'number' ||
        rawEquipment.maxDurability < rawEquipment.durability
      ) {
  ...
  private validateWeaponEffects(rawEffects: any[]): DataLoadResult<WeaponEffect[]> {
    try {
      ...
      for (const rawEffect of rawEffects) {
        ...
        if (
          typeof rawEffect.chance !== 'number' ||
          rawEffect.chance < 0 ||
          rawEffect.chance > 100
        ) {
  ...
  public repairWeapon(weapon: Weapon, repairAmount?: number): Weapon {
    ...
    if (repairAmount !== undefined) {
      updatedWeapon.durability = Math.min(
        updatedWeapon.maxDurability,
        updatedWeapon.durability + repairAmount
      );
    } else {
