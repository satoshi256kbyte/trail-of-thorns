...
export class DataValidator {
    ...
    private static validateMapLayers(mapData: MapData, result: ValidationResult): void {
        ...
        mapData.layers.forEach((layer, index) => {
            if (layer.data.length !== mapData.height) {
                result.errors.push(`Layer ${index} (${layer.name}) height mismatch: expected ${mapData.height}, got ${layer.data.length}`);
                result.isValid = false;
            }

            layer.data.forEach((row, rowIndex) => {
                if (row.length !== mapData.width) {
                    result.errors.push(`Layer ${index} (${layer.name}) row ${rowIndex} width mismatch: expected ${mapData.width}, got ${row.length}`);
                    result.isValid = false;
                }
            });
        });
        ...
        for (let i = 0; i < allSpawns.length; i++) {
            for (let j = i + 1; j < allSpawns.length; j++) {
                if (allSpawns[i].x === allSpawns[j].x && allSpawns[i].y === allSpawns[j].y) {
                    result.errors.push(`Spawn point overlap at (${allSpawns[i].x}, ${allSpawns[i].y}) between ${allSpawns[i].type} and ${allSpawns[j].type}`);
                    result.isValid = false;
                }
            }
        }
     ...
     */
    private static validateUnits(units: Unit[], faction: 'player' | 'enemy', mapData: MapData, result: ValidationResult): void {
        if (units.length === 0) {
     ...
     */
    private static validateUnit(unit: Unit, index: number, faction: string, mapData: MapData, result: ValidationResult): void {
        // Check position bounds
        const boundsResult = this.validateMapBounds(unit.position, mapData);
        if (!boundsResult.isValid) {
            result.errors.push(`${faction} unit ${index} (${unit.name}) position is out of bounds: ${boundsResult.error}`);
            result.isValid = false;
        }

        // Check faction consistency
        if (unit.faction !== faction) {
            result.errors.push(`${faction} unit ${index} (${unit.name}) has incorrect faction: ${unit.faction}`);
            result.isValid = false;
        }

        // Validate stats ranges
        if (unit.stats.maxHP > 9999) {
            result.warnings.push(`${faction} unit ${index} (${unit.name}) has very high HP: ${unit.stats.maxHP}`);
        }

        if (unit.stats.movement > 10) {
            result.warnings.push(`${faction} unit ${index} (${unit.name}) has very high movement: ${unit.stats.movement}`);
        }

        // Check for reasonable stat values
        if (unit.stats.speed <= 0) {
            result.errors.push(`${faction} unit ${index} (${unit.name}) has invalid speed: ${unit.stats.speed}`);
            result.isValid = false;
        }
    }
     ...
     */
    private static validateVictoryConditions(conditions: VictoryCondition[], stage: StageData, result: ValidationResult): void {
            ...
            switch (condition.type) {
                case 'reach_position':
                    if (!condition.position) {
                        ...
                        if (!boundsResult.isValid) {
                            result.errors.push(`Victory condition ${index} position out of bounds: ${boundsResult.error}`);
                            result.isValid = false;
                        }
                    }
                    break;
                ...
                case 'protect_unit':
                    if (!condition.target) {
                        ...
                        const unitExists = [...stage.playerUnits, ...stage.enemyUnits].some(unit => unit.id === condition.target);
                        if (!unitExists) {
                            result.errors.push(`Victory condition ${index} references non-existent unit: ${condition.target}`);
                            result.isValid = false;
                        }
                    }
                    break;
                ...
                case 'survive_turns':
                    if (!condition.turns || condition.turns <= 0) {
                        result.errors.push(`Victory condition ${index} (survive_turns) has invalid turn count: ${condition.turns}`);
                        result.isValid = false;
                    }
                    break;
            }
    ...
    private static performCrossValidation(stage: StageData, result: ValidationResult): void {
        ...
        if (stage.playerUnits.length > stage.mapData.playerSpawns.length) {
            result.warnings.push(`More player units (${stage.playerUnits.length}) than spawn points (${stage.mapData.playerSpawns.length})`);
        }

        if (stage.enemyUnits.length > stage.mapData.enemySpawns.length) {
            result.warnings.push(`More enemy units (${stage.enemyUnits.length}) than spawn points (${stage.mapData.enemySpawns.length})`);
        }
    ...
    private static checkUnitPositionOverlaps(stage: StageData, result: ValidationResult): void {
        ...
        for (let i = 0; i < allUnits.length; i++) {
            for (let j = i + 1; j < allUnits.length; j++) {
                if (allUnits[i].position.x === allUnits[j].position.x &&
                    allUnits[i].position.y === allUnits[j].position.y) {
                    result.errors.push(`Unit position overlap at (${allUnits[i].position.x}, ${allUnits[i].position.y}) between ${allUnits[i].name} and ${allUnits[j].name}`);
                    result.isValid = false;
                }
            }
        }
    ...
    private static validateGameBalance(stage: StageData, result: ValidationResult): void {
        ...
        if (enemyCount > playerCount * 3) {
            result.warnings.push(`Enemy units (${enemyCount}) significantly outnumber player units (${playerCount})`);
        }
