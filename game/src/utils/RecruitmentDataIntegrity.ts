...
export class RecruitmentDataIntegrityChecker {
    ...
    private async performCheck(
        ...
        const isValid = this.errors.filter(e => e.type === 'CRITICAL' || e.type === 'ERROR').length === 0;
    ...
    private checkCharacterDataIntegrity(): void {
        ...
        this.characters.forEach((character, index) => {
            ...
            if (characterIds.has(character.id)) {
                this.addError('CRITICAL', 'DUPLICATE_CHARACTER_ID',
                    `Duplicate character ID: ${character.id}`, context);
            }
            characterIds.add(character.id);

            // Check recruitment data consistency
            if (character.isRecruitable && !character.recruitmentData) {
                this.addError('ERROR', 'MISSING_RECRUITMENT_DATA',
                    `Character ${character.id} is marked as recruitable but has no recruitment data`, context);
            }

            if (!character.isRecruitable && character.recruitmentData) {
                this.addWarning('INCONSISTENT_RECRUITMENT_FLAG',
                    `Character ${character.id} has recruitment data but is not marked as recruitable`, context);
            }
    ...
    private checkStageDataIntegrity(): void {
        ...
        this.stages.forEach((stage, index) => {
            ...
            if (stageIds.has(stage.id)) {
                this.addError('CRITICAL', 'DUPLICATE_STAGE_ID',
                    `Duplicate stage ID: ${stage.id}`, context);
            }
            stageIds.add(stage.id);

            // Check for duplicate stage orders
            if (stageOrders.has(stage.order)) {
                this.addError('ERROR', 'DUPLICATE_STAGE_ORDER',
                    `Duplicate stage order: ${stage.order}`, context);
            }
            stageOrders.add(stage.order);

            // Check map data validity
            if (stage.mapData.width < 8 || stage.mapData.height < 6) {
                this.addWarning('SMALL_MAP_SIZE',
                    `Stage ${stage.id} has very small map size (${stage.mapData.width}x${stage.mapData.height})`,
                    context, 'Consider increasing map size for better gameplay');
            }
        ...
        this.stages.forEach(stage => {
            ...
            stage.playerUnits.forEach((unit, index) => {
                if (!characterIds.has(unit.characterId)) {
                    this.addError('ERROR', 'INVALID_PLAYER_UNIT_REFERENCE',
            ...
            stage.enemyUnits.forEach((unit, index) => {
                if (!characterIds.has(unit.characterId)) {
                    this.addError('ERROR', 'INVALID_ENEMY_UNIT_REFERENCE',
            ...
            stage.recruitableCharacters.forEach((recruitable, index) => {
                if (!characterIds.has(recruitable.characterId)) {
                    this.addError('ERROR', 'INVALID_RECRUITABLE_REFERENCE',
                    ...
                    if (character && !character.isRecruitable) {
                        this.addError('ERROR', 'NON_RECRUITABLE_IN_STAGE',
        ...
        this.characters.forEach(character => {
            ...
            character.recruitmentData.conditions.forEach((condition, index) => {
                ...
                if (!Object.values(RecruitmentConditionType).includes(condition.type)) {
                    this.addError('ERROR', 'INVALID_CONDITION_TYPE',
                        `Invalid recruitment condition type: ${condition.type}`, context);
                    return;
                }
    ...
    private checkGameBalanceConsiderations(): void {
        ...
        if (recruitableCount === 0) {
            this.addWarning('NO_RECRUITABLE_CHARACTERS',
        ...
        if (recruitableCount > totalEnemies * 0.8) {
            this.addWarning('HIGH_RECRUITMENT_RATIO',
        ...
        const stagesWithRecruitment = this.stages.filter(s => s.recruitableCharacters.length > 0).length;
        if (stagesWithRecruitment < this.stages.length * 0.3) {
            this.addWarning('LOW_RECRUITMENT_STAGES',
    ...
    private validateCharacterStats(character: CharacterData): void {
        ...
        if (stats.maxHP <= 0) {
            this.addError('ERROR', 'INVALID_HP', `Character ${character.id} has invalid HP: ${stats.maxHP}`, context);
        }

        if (stats.maxMP < 0) {
            this.addError('ERROR', 'INVALID_MP', `Character ${character.id} has invalid MP: ${stats.maxMP}`, context);
        }

        if (stats.attack <= 0) {
            this.addError('ERROR', 'INVALID_ATTACK', `Character ${character.id} has invalid attack: ${stats.attack}`, context);
        }

        if (stats.defense < 0) {
            this.addError('ERROR', 'INVALID_DEFENSE', `Character ${character.id} has invalid defense: ${stats.defense}`, context);
        }

        if (stats.speed <= 0) {
            this.addError('ERROR', 'INVALID_SPEED', `Character ${character.id} has invalid speed: ${stats.speed}`, context);
        }

        if (stats.movement <= 0) {
            this.addError('ERROR', 'INVALID_MOVEMENT', `Character ${character.id} has invalid movement: ${stats.movement}`, context);
        }
    }
    ...
    private validateUnitPlacements(stage: StageData): void {
        ...
        allUnits.forEach((unit, index) => {
            ...
            if (occupiedPositions.has(posKey)) {
                this.addError('ERROR', 'POSITION_CONFLICT',
            ...
            if (unit.startPosition.x < 0 || unit.startPosition.x >= stage.mapData.width ||
                unit.startPosition.y < 0 || unit.startPosition.y >= stage.mapData.height) {
                this.addError('ERROR', 'OUT_OF_BOUNDS_PLACEMENT',
        ...
        switch (condition.type) {
            case RecruitmentConditionType.SPECIFIC_ATTACKER:
                if (!condition.parameters.attackerId) {
                    this.addError('ERROR', 'MISSING_ATTACKER_ID',
                        'specific_attacker condition missing attackerId parameter', context);
                } else {
                    const attackerExists = this.characters.some(c => c.id === condition.parameters.attackerId);
                    if (!attackerExists) {
                        this.addError('ERROR', 'INVALID_ATTACKER_REFERENCE',
                            `specific_attacker condition references unknown character: ${condition.parameters.attackerId}`, context);
                    }
                }
                break;
            ...
            case RecruitmentConditionType.HP_THRESHOLD:
                if (typeof condition.parameters.threshold !== 'number' ||
                    condition.parameters.threshold <= 0 || condition.parameters.threshold > 1) {
                    this.addError('ERROR', 'INVALID_HP_THRESHOLD',
                        `hp_threshold condition has invalid threshold: ${condition.parameters.threshold}`, context);
                }
                break;

            case RecruitmentConditionType.TURN_LIMIT:
                if (typeof condition.parameters.maxTurn !== 'number' || condition.parameters.maxTurn < 1) {
                    this.addError('ERROR', 'INVALID_TURN_LIMIT',
                        `turn_limit condition has invalid maxTurn: ${condition.parameters.maxTurn}`, context);
                }
                break;

            case RecruitmentConditionType.ALLY_PRESENT:
                if (!condition.parameters.requiredAllyId) {
                    this.addError('ERROR', 'MISSING_ALLY_ID',
                        'ally_present condition missing requiredAllyId parameter', context);
                } else {
                    const allyExists = this.characters.some(c => c.id === condition.parameters.requiredAllyId);
                    if (!allyExists) {
                        this.addError('ERROR', 'INVALID_ALLY_REFERENCE',
                            `ally_present condition references unknown character: ${condition.parameters.requiredAllyId}`, context);
                    }
                }
                break;
        }
