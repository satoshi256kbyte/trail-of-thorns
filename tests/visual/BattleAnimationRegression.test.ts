...
describe('Battle Animation - Visual Regression Tests', () => {
  ...
  describe('ダメージエフェクトと状態同期', () => {
    ...
    test('クリティカルヒット時の特殊エフェクト', async () => {
      ...
      await battleAnimator.showDamageEffect(mockTarget, damage, DamageType.CRITICAL);
    ...
    test('回復エフェクトの視覚表現', async () => {
      ...
      await battleAnimator.showDamageEffect(mockTarget, healAmount, DamageType.HEALING);
  ...
  describe('視覚的フィードバックの品質', () => {
    test('攻撃範囲ハイライトの視覚的明瞭性', async () => {
      ...
      attackRange.forEach((_, index) => {
        expect(mockScene.add.graphics).toHaveBeenNthCalledWith(index + 1, expect.any(Object));
      });
    });
