        ...
        json: {
            get: jest.fn(() => ({
                ...
                tiles: Array(8).fill(null).map(() =>
                    Array(12).fill({ type: 'grass', movementCost: 1 })
                )
            }))
        }
    },
...
describe('GameplayScene Battle Integration', () => {
    ...
    const createMockUnit = (id: string, faction: 'player' | 'enemy', position = { x: 1, y: 1 }): Unit => ({
        id,
        name: `Test ${id}`,
        position,
        stats: {
    ...
    const createMockStageData = (): StageData => ({
        ...
        victoryConditions: [
            { type: 'defeat_all', description: 'Defeat all enemies' }
        ]
    });
        ...
        test('should disable attack option when unit cannot attack', () => {
            ...
            expect(mockUIManager.showActionMenu).toHaveBeenCalledWith(
                expect.not.arrayContaining([
                    expect.objectContaining({ text: 'Attack', enabled: true })
                ])
            );
        });
    });
