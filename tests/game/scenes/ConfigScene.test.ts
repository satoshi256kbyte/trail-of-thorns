...
describe('ConfigScene', () => {
    ...
    describe('Scene Navigation', () => {
        ...
        test('should handle back button navigation with fade transition', () => {
            ...
            const backButtonCall = (MenuButton as jest.Mock).mock.calls.find(
                call => call[3] === 'Back'
            );
    ...
    describe('Error Handling', () => {
        test('should handle errors in background setup gracefully', () => {
            ...
            const mockGraphics = {
                fillGradientStyle: jest.fn(() => { throw new Error('Test error'); }),
                fillRect: jest.fn(),
                setDepth: jest.fn(),
                destroy: jest.fn(),
            };
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith('Error setting up background:', expect.any(Error));
        ...
        test('should handle errors in title creation gracefully', () => {
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith('Error creating config title:', expect.any(Error));
            consoleErrorSpy.mockRestore();
        });
        ...
        test('should handle errors in back button handling gracefully', () => {
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith('Error handling back button:', expect.any(Error));
            consoleErrorSpy.mockRestore();
        });
    });
