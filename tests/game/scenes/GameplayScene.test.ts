...
describe('GameplayScene', () => {
        ...
        mapData: {
            ...
            layers: [
                ...
                },
                {
                    ...
                    data: Array(8).fill(null).map(() => Array(12).fill(0)),
                    visible: true,
                    opacity: 1.0,
                }
            ],
    ...
    const measurePerformance = (fn: Function) => {
        ...
        performanceMetrics.averageUpdateTime = performanceMetrics.totalUpdateTime / performanceMetrics.frameCount;
    ...
    describe('Create Method', () => {
        ...
        test('should log create phase and completion messages', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('GameplayScene: initialization completed successfully');
        });
    ...
    describe('Performance Testing', () => {
        ...
        test('should handle rapid scene creation and destruction', () => {
            ...
            const avgDestructionTime = destructionTimes.reduce((a, b) => a + b, 0) / destructionTimes.length;
    ...
    describe('Complete Gameplay Flow Integration', () => {
        test('should handle complete turn-based gameplay cycle', async () => {
            ...
            // Verify state after turn change
            const newState = gameStateManager.getGameState();
            expect(newState.currentTurn).toBeGreaterThan(initialState.currentTurn);
        });
        ...
        test('should handle character selection and interaction flow', async () => {
            ...
            expect(selectedUnit?.id).toBe(testUnit.id);
    ...
    describe('Error Recovery and Resilience', () => {
        ...
        test('should handle corrupted stage data gracefully', async () => {
            ...
            // Should fall back to default stage data
            const stageData = scene.getStageData();
            expect(stageData).toBeDefined();
            expect(stageData?.id).toBe('default-stage');
        });
    ...
    describe('Accessibility and User Experience', () => {
        test('should provide keyboard navigation support', async () => {
            ...
            // Test that input handler was configured for keyboard
            const { InputHandler } = require('../../../game/src/input/InputHandler');
        ...
        test('should provide visual feedback for user actions', async () => {
            ...
            // Verify UI manager was set up for visual feedback
            const { UIManager } = require('../../../game/src/ui/UIManager');
            const uiManagerInstance = UIManager.mock.instances[0];
            expect(uiManagerInstance.createUI).toHaveBeenCalled();
            expect(uiManagerInstance.updateUI).toHaveBeenCalled();
        });
