...
import {
  ...
  ValidationResult,
  MapBoundsResult,
} from '../../../game/src/utils/DataValidator';
import {
  StageData,
  MapData,
  Unit,
...
describe('DataValidator', () => {
  ...
  const createValidMapData = (): MapData => ({
    ...
    height: 10,
    tileSize: 32,
    layers: [createValidMapLayer('background'), createValidMapLayer('terrain')],
  ...
  describe('validateStageData', () => {
    ...
    it('should warn about special characters in ID', () => {
      ...
      expect(result.warnings).toContain(
        'Stage ID contains special characters that may cause issues'
      );
    });
    ...
    it('should warn about very small maps', () => {
      ...
      expect(result.warnings).toContain(
        'Map is very small (less than 5x5), gameplay may be limited'
      );
    });
    ...
    it('should warn about very large maps', () => {
      ...
      expect(result.warnings).toContain(
        'Map is very large (more than 50x50), performance may be affected'
      );
    });
  });
  ...
  describe('map layer validation', () => {
    it('should reject layers with mismatched dimensions', () => {
      const stageData = createValidStageData();
      stageData.mapData.layers[0].data = Array(5)
        .fill(null)
        .map(() => Array(10).fill(0)); // Wrong height
  ...
  describe('spawn point validation', () => {
    ...
    it('should reject spawn points out of bounds', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some(error => error.includes('Player spawn 0 is out of bounds'))).toBe(
        true
      );
    });
  ...
  describe('victory condition validation', () => {
    ...
    it('should reject protect_unit condition with non-existent target', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some(error => error.includes('references non-existent unit'))).toBe(
        true
      );
    });
  ...
  describe('unit validation', () => {
    ...
    it('should warn about very high movement', () => {
      ...
      expect(result.warnings.some(warning => warning.includes('has very high movement'))).toBe(
        true
      );
    });
  });
  ...
  describe('balance validation', () => {
    it('should warn about enemy units significantly outnumbering players', () => {
      ...
      for (let i = 3; i <= 10; i++) {
        stageData.enemyUnits.push(
          createValidUnit(`enemy${i}`, 'enemy', i % 10, 7 + Math.floor(i / 10))
        );
      }
      ...
      expect(
        result.warnings.some(warning => warning.includes('significantly outnumber player units'))
      ).toBe(true);
    });
