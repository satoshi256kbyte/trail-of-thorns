...
describe('GameplayErrorHandler', () => {
  ...
  describe('error logging', () => {
    it('should log errors to internal log', () => {
      errorHandler.handleError(GameplayError.INVALID_ACTION, 'Test error message');
    ...
    it('should limit error log size to 100 entries', () => {
      // Add 150 errors
      for (let i = 0; i < 150; i++) {
        errorHandler.handleError(GameplayError.INVALID_ACTION, `Error ${i}`);
      }
  ...
  describe('error severity', () => {
    it('should assign correct severity levels', () => {
      const criticalErrors = [GameplayError.INVALID_STAGE_DATA, GameplayError.MAP_LOAD_FAILED];

      const highErrors = [GameplayError.CHARACTER_LOAD_FAILED, GameplayError.INVALID_TURN_STATE];

      const mediumErrors = [GameplayError.UNIT_NOT_FOUND, GameplayError.INVALID_ACTION];

      const lowErrors = [GameplayError.CAMERA_BOUNDS_ERROR, GameplayError.INVALID_POSITION];
  ...
  describe('recovery actions', () => {
    ...
    it('should execute successful recovery action', async () => {
      const mockAction = jest.fn().mockResolvedValue(true);
      const recoveryAction = errorHandler.createRecoveryAction('retry', 'Test action', mockAction);
  ...
  describe('convenience functions', () => {
    it('should handle error through convenience function', () => {
      const result = handleGameplayError(GameplayError.INVALID_ACTION, 'Test error message', {
        scene: 'TestScene',
      });
    ...
    it('should create recovery action through convenience function', () => {
      const mockAction = jest.fn().mockResolvedValue(true);
      const recoveryAction = createRecoveryAction('fallback', 'Test fallback', mockAction);
  ...
  describe('error context handling', () => {
    ...
    it('should handle empty context', () => {
      const result = errorHandler.handleError(GameplayError.INVALID_ACTION, 'Test error');
  ...
  describe('recovery scheduling', () => {
    ...
    it('should schedule scene transitions', () => {
      ...
      errorHandler.handleError(GameplayError.INVALID_STAGE_DATA, 'Test error', {
        scene: 'GameplayScene',
      });
    ...
    it('should schedule retries', () => {
      ...
      errorHandler.handleError(GameplayError.CHARACTER_LOAD_FAILED, 'Test error');
    ...
    it('should schedule various recovery actions', () => {
      ...
      errorHandler.handleError(GameplayError.INVALID_ACTION, 'Test', {
        system: 'GameStateManager',
      });
      errorHandler.handleError(GameplayError.CAMERA_BOUNDS_ERROR, 'Test', {
        system: 'CameraController',
      });
