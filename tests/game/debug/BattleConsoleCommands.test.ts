import {
  BattleConsoleCommands,
  CommandResult,
  MockUnitOptions,
} from '../../../game/src/debug/BattleConsoleCommands';
...
describe('BattleConsoleCommands', () => {
  ...
  describe('Battle Testing Commands', () => {
    ...
    test('should simulate battle multiple times', () => {
      const result = (global.window as any).battleCommands.simulateBattle(
        'attacker',
        'target',
        500
      );
    ...
    test('should test damage calculation details', () => {
      const result = (global.window as any).battleCommands.testDamageCalculation(
        'attacker',
        'target'
      );
  ...
  describe('Help Command', () => {
    test('should display help information', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('戦闘コンソールコマンド ヘルプ')
      );
