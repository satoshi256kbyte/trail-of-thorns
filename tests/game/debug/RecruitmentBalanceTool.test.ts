...
describe('RecruitmentBalanceTool', () => {
  ...
  describe('Balance Testing', () => {
    ...
    it('should handle test configuration not found', async () => {
      await expect(
        balanceTool.runBalanceTest(mockCharacter, mockAttacker, mockTarget, 'nonexistent')
    ...
    it('should generate appropriate recommendations for low success rate', async () => {
      ...
      expect(result.recommendations.some(rec => rec.includes('Success rate too low'))).toBe(true);
    });
    ...
    it('should generate appropriate recommendations for high success rate', async () => {
      ...
      expect(result.recommendations.some(rec => rec.includes('Success rate too high'))).toBe(true);
    });
    ...
    it('should track condition effectiveness', async () => {
      ...
      await balanceTool.runBalanceTest(mockCharacter, mockAttacker, mockTarget, 'standard');
  ...
  describe('Comprehensive Analysis', () => {
    it('should run comprehensive analysis', async () => {
      ...
      const report = await balanceTool.runComprehensiveAnalysis(characters, testUnits, [
        'standard',
      ]);
    ...
    it('should generate global recommendations', async () => {
      ...
      expect(
        report.globalRecommendations.some(rec => rec.includes('success rates are too low'))
      ).toBe(true);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle simulation errors gracefully', async () => {
      ...
      await expect(
        balanceTool.runBalanceTest(mockCharacter, mockAttacker, mockTarget, 'standard')
  ...
  describe('Progress Reporting', () => {
    it('should report progress for long tests', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Balance test completed'));
    });
  });
});
