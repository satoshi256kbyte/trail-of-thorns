...
describe('BattleStateManager Integration', () => {
  ...
  describe('complete battle flow integration', () => {
    test('should handle complete battle sequence with damage calculation', () => {
      ...
      const baseDamage = damageCalculator.calculateBaseDamage(
        playerUnit,
        enemyUnit,
        playerUnit.weapon!
      );
      ...
      const postBattleResult = battleStateManager.updatePostBattle(battleResult, [
        playerUnit,
        enemyUnit,
      ]);
    ...
    test('should handle unit defeat and experience granting', () => {
      ...
      expect((playerUnit as any).experience).toBeGreaterThanOrEqual(
        50 + expConfig.defeatExperience
      );
    });
