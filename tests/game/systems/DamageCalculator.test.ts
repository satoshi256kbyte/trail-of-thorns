...
} from '../../../game/src/systems/DamageCalculator';
...
describe('DamageCalculator', () => {
  ...
  describe('calculateBaseDamage', () => {
    ...
    test('should handle different damage formulas', () => {
      const linearCalculator = new DamageCalculator({
        baseDamageFormula: 'linear',
        enableVariance: false,
      });
      const sqrtCalculator = new DamageCalculator({
        baseDamageFormula: 'square_root',
        enableVariance: false,
      });
      const logCalculator = new DamageCalculator({
        baseDamageFormula: 'logarithmic',
        enableVariance: false,
      });
  ...
  describe('applyElementalModifier', () => {
    test('should apply no modifier for same elements', () => {
      ...
      const modifiedDamage = calculator.applyElementalModifier(
        baseDamage,
        Element.FIRE,
        Element.FIRE
      );
    ...
    test('should apply effectiveness modifier for different elements', () => {
      ...
      // Fire vs Water (weak)
      const fireVsWater = calculator.applyElementalModifier(
        baseDamage,
        Element.FIRE,
        Element.WATER
      );
      ...
      const fireVsEarth = calculator.applyElementalModifier(
        baseDamage,
        Element.FIRE,
        Element.EARTH
      );
      ...
      const lightVsDark = calculator.applyElementalModifier(
        baseDamage,
        Element.LIGHT,
        Element.DARK
      );
  ...
  describe('performCompleteCalculation', () => {
    ...
    test('should include additional modifiers', () => {
      ...
      const result = calculator.performCompleteCalculation(
        attacker,
        target,
        weapon,
        additionalModifiers
  ...
  describe('utility methods', () => {
    ...
    test('should calculate healing amount', () => {
      ...
      expect(healingAmount).toBeLessThanOrEqual(
        damagedTarget.stats.maxHP - damagedTarget.currentHP
      );
    });
  ...
  describe('edge cases', () => {
    ...
    test('should handle same speed units', () => {
      const sameSpeedTarget = {
        ...target,
        stats: { ...target.stats, speed: attacker.stats.speed },
      };
