...
describe('TargetSelector', () => {
    ...
    beforeEach(() => {
        ...
        mockWeapon = {
            ...
            rangePattern: {
                type: 'single',
                range: 1,
                pattern: [{ x: 1, y: 0 }, { x: -1, y: 0 }, { x: 0, y: 1 }, { x: 0, y: -1 }],
            },
        ...
        mockAttackRangeCalculator.calculateAttackRange.mockReturnValue({
            validPositions: [{ x: 6, y: 5 }, { x: 7, y: 5 }, { x: 4, y: 5 }, { x: 5, y: 4 }, { x: 5, y: 6 }],
            blockedPositions: [],
            weapon: mockWeapon,
        ...
        mockAttackRangeCalculator.calculateAreaOfEffect.mockReturnValue([
            { x: 7, y: 5 }, { x: 6, y: 5 }, { x: 8, y: 5 }, { x: 7, y: 4 }, { x: 7, y: 6 }
        ]);
    ...
    describe('initializeSelection', () => {
        ...
        test('should throw error for invalid attacker', () => {
            ...
            expect(() => {
                targetSelector.initializeSelection(
                    invalidAttacker,
                    mockWeapon,
                    mockAllUnits,
                    mockMapData
    ...
    describe('getValidTargets', () => {
        ...
        test('should return only enemy targets by default', () => {
            const attackRange = [{ x: 6, y: 5 }, { x: 7, y: 5 }];
        ...
        test('should exclude attacker from valid targets', () => {
            const attackRange = [{ x: 5, y: 5 }, { x: 6, y: 5 }, { x: 7, y: 5 }];
            const validTargets = targetSelector.getValidTargets(mockAttacker, attackRange);
    ...
    describe('getAreaTargets', () => {
        ...
        test('should respect max targets per attack limit', () => {
            ...
            limitedSelector.initializeSelection(mockAttacker, mockAreaWeapon, allUnitsWithMany, mockMapData);
        ...
        test('should include friendly units when friendly fire is allowed', () => {
            ...
            friendlyFireSelector.initializeSelection(mockAttacker, mockAreaWeapon, mockAllUnits, mockMapData);
    ...
    describe('getOptimalTarget', () => {
        ...
        beforeEach(() => {
            ...
            mockAttackRangeCalculator.calculateAttackRange.mockReturnValue({
                validPositions: [
                    { x: 6, y: 5 }, { x: 8, y: 8 }, { x: 7, y: 5 }, { x: 7, y: 6 }
                ],
                blockedPositions: [],
                weapon: mockWeapon,
                attacker: mockAttacker,
