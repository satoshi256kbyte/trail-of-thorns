...
const createMockCondition = (
  overrides: Partial<RecruitmentCondition> = {}
): RecruitmentCondition => ({
  id: 'test-condition',
...
describe('RecruitmentUI', () => {
  ...
  describe('showRecruitmentConditions', () => {
    test('should display recruitment conditions for a unit', () => {
      ...
      const mockTitle = mockScene.add.text.mock.results.find(result => result.value.setText)?.value;
      ...
      // Should emit event
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('recruitment-conditions-shown', {
        unitId: mockUnit.id,
  ...
  describe('updateRecruitmentProgress', () => {
    ...
    test('should update progress bar and text', () => {
      ...
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('recruitment-progress-updated', {
        unitId: mockUnit.id,
        progress: 50,
        isEligible: false,
      });
  ...
  describe('showNPCIndicator', () => {
    test('should create and display NPC indicator', () => {
      ...
      // Should emit event
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('npc-indicator-shown', {
        unitId: mockUnit.id,
      });
    });
    ...
    test('should replace existing indicator when called multiple times', () => {
      ...
      const firstContainer =
        mockScene.add.container.mock.results[mockScene.add.container.mock.results.length - 1].value;
  ...
  describe('hideNPCIndicator', () => {
    test('should hide and destroy NPC indicator', () => {
      ...
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('npc-indicator-hidden', {
        unitId: mockUnit.id,
      });
    });
  ...
  describe('showRecruitmentSuccess', () => {
    test('should display success notification with animation', () => {
      ...
      // Should emit event
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('recruitment-success-shown', {
        unitId: mockUnit.id,
      });
    });
  ...
  describe('showRecruitmentFailure', () => {
    ...
    test('should display failure notification with shake animation', () => {
      ...
      // Should emit event
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('recruitment-failure-shown', {
        unitId: mockUnit.id,
