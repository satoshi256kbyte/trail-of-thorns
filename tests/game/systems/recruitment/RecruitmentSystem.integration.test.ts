...
describe('RecruitmentSystem Integration Tests', () => {
    ...
    beforeEach(() => {
        ...
        mockStageData = {
            ...
            mapData: {
                ...
                playerSpawns: [{ x: 0, y: 0 }, { x: 1, y: 0 }],
                enemySpawns: [{ x: 8, y: 8 }, { x: 6, y: 6 }, { x: 7, y: 5 }]
            },
    ...
    describe('Complete Recruitment Workflow', () => {
        it('should handle full recruitment workflow for simple conditions', async () => {
            ...
            expect(mockEventEmitter.emit).toHaveBeenCalledWith('recruitment-initialized', expect.any(Object));
            expect(mockEventEmitter.emit).toHaveBeenCalledWith('recruitment-eligibility-checked', expect.any(Object));
            expect(mockEventEmitter.emit).toHaveBeenCalledWith('recruitment-attempt-processed', expect.any(Object));
            expect(mockEventEmitter.emit).toHaveBeenCalledWith('character-converted-to-npc', expect.any(Object));
            expect(mockEventEmitter.emit).toHaveBeenCalledWith('recruitment-completed', expect.any(Object));
            expect(mockEventEmitter.emit).toHaveBeenCalledWith('stage-recruitment-completed', expect.any(Object));
        });
    ...
    describe('Error Handling and Edge Cases', () => {
        ...
        it('should handle NPC limit correctly', async () => {
            ...
            knight.currentHP = 35;
            const firstResult = recruitmentSystem.processRecruitmentAttempt(hero, knight, 40, undefined, 1);
