...
describe('BattleSystem Debug Integration', () => {
  ...
  beforeEach(() => {
    ...
    mockMapData = {
      ...
      tiles: Array(10)
        .fill(null)
        .map(() => Array(10).fill({ type: 'grass', movementCost: 1, isPassable: true })),
      units: mockUnits,
    };
  ...
  describe('Debug Manager Integration', () => {
    ...
    test('should generate debug report', () => {
      ...
      const reportSpy = jest
        .spyOn(debugManager, 'generateDebugReport')
        .mockReturnValue('Test report');
  ...
  describe('Performance Monitoring Integration', () => {
    test('should record performance metrics during debug logging', () => {
      ...
      const timestamp2 = debugManager.logBattlePhaseStart(
        'damage_calculation',
        mockUnits[0],
        mockUnits[1]
      );
  ...
  describe('Error Handling with Debug Integration', () => {
    test('should log errors in debug system', () => {
      ...
      const timestamp = debugManager.logBattlePhaseStart(
        'damage_calculation',
        mockUnits[0],
        mockUnits[1]
      );
      ...
      expect(logEndSpy).toHaveBeenCalledWith(timestamp, undefined, undefined, 'Test error message');
    });
