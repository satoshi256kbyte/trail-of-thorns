...
import {
  RecruitmentDataManager,
  RecruitedCharacterSaveData,
  ChapterProgressData,
  RecruitmentSaveData,
} from '../../../game/src/systems/RecruitmentDataManager';
import {
...
describe('RecruitmentDataManager', () => {
  ...
  beforeEach(() => {
    ...
    mockRecruitedCharacters = [
      ...
      },
      {
        ...
        level: 1,
        isAvailable: true,
        recruitmentConditions: [{ type: 'hp_threshold', parameters: { threshold: 0.3 } }],
      },
    ];
  ...
  describe('markCharacterLost', () => {
    ...
    test('should mark character as lost in chapter', async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('recruited-character-lost', {
        characterId: 'knight_01',
        chapterId: 'chapter_2',
      });
  ...
  describe('resetChapterLossStatus', () => {
    ...
    test('should reset loss status and restore character availability', async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('chapter-loss-status-reset', {
        chapterId: 'chapter_2',
        restoredCharacters: ['knight_01'],
      });
  ...
  describe('addRecruitedCharacter', () => {
    beforeEach(async () => {
      await dataManager.saveRecruitmentData([], {}, mockStatistics, mockConfig, 'chapter_1');
      await dataManager.loadRecruitmentData();
    });
    ...
    test('should add newly recruited character', async () => {
      ...
      const result = await dataManager.addRecruitedCharacter(mockUnit, 'chapter_1', 'stage_1_2', [
        { type: 'specific_attacker', parameters: { attackerId: 'protagonist' } },
      ]);

      expect(result.success).toBe(true);
      expect(mockEventEmitter.emit).toHaveBeenCalledWith('character-recruited-saved', {
        characterId: 'archer_01',
        chapterId: 'chapter_1',
        stageId: 'stage_1_2',
      });
