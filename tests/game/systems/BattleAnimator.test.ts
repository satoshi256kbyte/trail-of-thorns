...
describe('BattleAnimator', () => {
  ...
  describe('playDefeatedAnimation', () => {
    ...
    test('should throw error if sprite is missing', async () => {
      ...
      await expect(battleAnimator.playDefeatedAnimation(unitWithoutSprite)).rejects.toThrow(
        'Missing sprite for defeat animation'
      );
    });
  ...
  describe('Particle Effects', () => {
    ...
    test('should not create particle effects when disabled', async () => {
      ...
      const damagePromise = animatorWithoutParticles.showDamageEffect(
        mockUnit2,
        25,
        DamageType.PHYSICAL
      );
