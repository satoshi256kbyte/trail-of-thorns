...
function createRealisticMapData(width: number = 10, height: number = 10): MapData {
  return {
    ...
    tiles: Array(height)
      .fill(null)
      .map(() => Array(width).fill({ type: 'grass', movementCost: 1 })),
  };
}
...
describe('AIController Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    aiController = new AIController(mockScene, battleSystem, movementSystem, {
  ...
  describe('Complete NPC Protection Scenario', () => {
    test('should prioritize NPC protection in complex battle scenario', async () => {
      ...
      if (decision1.type === AIActionType.ATTACK) {
        ...
        const distanceToNPC =
          Math.abs(newPos.x - npcUnit.position.x) + Math.abs(newPos.y - npcUnit.position.y);
        const currentDistanceToNPC =
          Math.abs(aiUnit1.position.x - npcUnit.position.x) +
          Math.abs(aiUnit1.position.y - npcUnit.position.y);
      ...
      if (decision2.type === AIActionType.ATTACK) {
        ...
        const distanceToNPC =
          Math.abs(newPos.x - npcUnit.position.x) + Math.abs(newPos.y - npcUnit.position.y);
        const currentDistanceToNPC =
          Math.abs(aiUnit2.position.x - npcUnit.position.x) +
          Math.abs(aiUnit2.position.y - npcUnit.position.y);
    ...
    test('should coordinate multiple AI units to target NPCs', async () => {
      ...
      const npcTargetingDecisions = decisions.filter(
        d => d.type === AIActionType.ATTACK && d.target?.id === npcUnit.id
      );
  ...
  describe('Tactical AI Behavior with NPCs', () => {
    test('should use tactical positioning when NPCs are present', async () => {
      ...
      if (decision.type === AIActionType.MOVE) {
        ...
        const currentDistance =
          Math.abs(aiUnit.position.x - npcUnit.position.x) +
          Math.abs(aiUnit.position.y - npcUnit.position.y);
    ...
    test('should balance between NPC targeting and self-preservation', async () => {
      ...
      if (decision.type === AIActionType.MOVE) {
        const newPos = decision.position!;
        const distanceFromThreat =
          Math.abs(newPos.x - strongPlayer.position.x) +
          Math.abs(newPos.y - strongPlayer.position.y);
        const currentDistanceFromThreat =
          Math.abs(lowHealthAI.position.x - strongPlayer.position.x) +
          Math.abs(lowHealthAI.position.y - strongPlayer.position.y);
