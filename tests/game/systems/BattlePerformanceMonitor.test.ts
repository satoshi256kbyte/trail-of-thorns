...
import { BattlePerformanceMonitor, PerformanceWarningType, OptimizationType } from '../../../game/src/systems/BattlePerformanceMonitor';
...
describe('BattlePerformanceMonitor', () => {
    ...
    describe('Monitoring Lifecycle', () => {
        ...
        test('should not start monitoring twice', () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('[BattlePerformanceMonitor] Performance monitoring started');
    ...
    describe('Frame Rate Monitoring', () => {
        ...
        test('should detect low frame rate warnings', async () => {
            ...
            const metrics = monitor.getMetrics();
            const lowFrameRateWarnings = metrics.warnings.filter(w => w.type === PerformanceWarningType.LOW_FRAMERATE);
    ...
    describe('Operation Timing', () => {
        ...
        test('should detect slow operations', () => {
            ...
            const metrics = monitor.getMetrics();
            const slowOpWarnings = metrics.warnings.filter(w => w.type === PerformanceWarningType.SLOW_OPERATION);
    ...
    describe('System Load Monitoring', () => {
        ...
        test('should detect high system load', () => {
            ...
            const metrics = monitor.getMetrics();
            const highLoadWarnings = metrics.warnings.filter(w => w.type === PerformanceWarningType.HIGH_SYSTEM_LOAD);
    ...
    describe('Memory Monitoring', () => {
        ...
        test('should detect high memory usage', async () => {
            ...
            const metrics = monitor.getMetrics();
            const memoryWarnings = metrics.warnings.filter(w => w.type === PerformanceWarningType.HIGH_MEMORY_USAGE);
        ...
        test('should detect memory leaks', async () => {
            ...
            const metrics = monitor.getMetrics();
            const leakWarnings = metrics.warnings.filter(w => w.type === PerformanceWarningType.MEMORY_LEAK);
    ...
    describe('Battle Completion Tracking', () => {
        ...
        test('should warn about long battles', () => {
            ...
            const metrics = monitor.getMetrics();
            const slowBattleWarnings = metrics.warnings.filter(w =>
    ...
    describe('Optimization Suggestions', () => {
        test('should generate optimization suggestions for low frame rate', async () => {
            ...
            const metrics = monitor.getMetrics();
            const animationOptimizations = metrics.optimizations.filter(opt =>
                opt.type === OptimizationType.REDUCE_ANIMATION_QUALITY
            );
        ...
        test('should generate memory optimization suggestions', async () => {
            ...
            const metrics = monitor.getMetrics();
            const poolingOptimizations = metrics.optimizations.filter(opt =>
                opt.type === OptimizationType.ENABLE_OBJECT_POOLING
            );
        ...
        test('should suggest cache optimization for slow calculations', () => {
            ...
            const metrics = monitor.getMetrics();
            const cacheOptimizations = metrics.optimizations.filter(opt =>
                opt.type === OptimizationType.INCREASE_CACHE_SIZE
            );
