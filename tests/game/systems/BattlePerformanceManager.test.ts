...
const createMockMapData = (): MapData => ({
  ...
  tiles: Array(10)
    .fill(null)
    .map(() => Array(10).fill({ type: 'grass', passable: true })),
  units: [],
});

describe('BattlePerformanceManager', () => {
  let performanceManager: BattlePerformanceManager;
  let mockCalculator: jest.MockedFunction<
    (attacker: Unit, weapon: Weapon, mapData?: MapData) => AttackRangeResult
  >;
  ...
  beforeEach(() => {
    ...
    mockCalculator = jest
      .fn()
      .mockImplementation((attacker: Unit, weapon: Weapon, mapData?: MapData) => ({
        validPositions: [
  ...
  describe('Attack Range Caching', () => {
    test('should cache attack range calculations', () => {
      ...
      const result1 = performanceManager.getCachedAttackRange(
        unit,
        weapon,
        mapData,
        mockCalculator
      ...
      const result2 = performanceManager.getCachedAttackRange(
        unit,
        weapon,
        mapData,
        mockCalculator
