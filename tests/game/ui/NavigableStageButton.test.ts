...
describe('NavigableStageButton', () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(NavigableStageButton.prototype as any, 'setupInteractivity')
      .mockImplementation(() => {});
  ...
  describe('Enhanced Visual Effects', () => {
    test('should apply pulse effect on focus for unlocked stages', () => {
      ...
      const scaleAnimation = (scene.tweens.add as jest.Mock).mock.calls.find(
        call => call[0].scaleX === 1.02 && call[0].scaleY === 1.02
      );
    ...
    test('should reset scale on blur', () => {
      ...
      const scaleResetAnimation = (scene.tweens.add as jest.Mock).mock.calls.find(
        call => call[0].scaleX === 1 && call[0].scaleY === 1
      );
