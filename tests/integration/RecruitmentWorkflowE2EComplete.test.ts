...
const createCompleteStageData = (): StageData => ({
    ...
    mapData: {
        ...
        tiles: Array(15).fill(null).map(() =>
            Array(15).fill({ type: 'grass', movementCost: 1 })
        )
    },
    playerUnits: [
        {
...
describe('仲間化システム E2E ワークフローテスト', () => {
    ...
    describe('完全な仲間化成功ワークフロー', () => {
        test('ステージ開始から仲間化完了までの完全フロー', async () => {
            ...
            const targetPriorities = [
                { unit: protagonist, priority: recruitmentSystem.getTargetPriority(regularEnemy, protagonist) },
                { unit: allyMage, priority: recruitmentSystem.getTargetPriority(regularEnemy, allyMage) },
                { unit: targetKnight, priority: recruitmentSystem.getTargetPriority(regularEnemy, targetKnight) }
            ];
    ...
    describe('仲間化失敗ワークフロー', () => {
        test('NPC撃破による仲間化失敗フロー', async () => {
            ...
            // 1. 仲間化条件を満たしてNPC化
            recruitmentSystem.processRecruitmentAttempt(protagonist, targetKnight, targetKnight.currentHP);
            expect(recruitmentSystem.isNPC(targetKnight)).toBe(true);
