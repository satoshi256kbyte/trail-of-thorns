...
export class MockStageFactory {
                ...
                layers: [
                    {
                        name: 'background',
                        type: 'background',
                        data: Array(6).fill(null).map(() => Array(8).fill(1)),
                        visible: true,
                        opacity: 1.0,
                    }
                ],
    ...
    static createLargeStage(): StageData {
        ...
        return {
            ...
            mapData: {
                ...
                layers: [
                    ...
                    },
                    {
                        name: 'terrain',
                        type: 'terrain',
                        data: Array(height).fill(null).map((_, y) =>
                ...
                playerSpawns: Array(playerCount).fill(null).map((_, i) => ({
                    x: 1 + (i % 4),
                    y: 12 + Math.floor(i / 4)
                })),
                enemySpawns: Array(enemyCount).fill(null).map((_, i) => ({
                    x: 15 + (i % 5),
                    y: 1 + Math.floor(i / 5)
                })),
            },
            playerUnits: Array(playerCount).fill(null).map((_, i) => ({
                id: `large-player-${i}`,
            ...
            enemyUnits: Array(enemyCount).fill(null).map((_, i) => ({
                id: `large-enemy-${i}`,
                name: `Enemy Unit ${i + 1}`,
                ...
                layers: [
                    {
                        name: 'background',
                        type: 'background',
                        data: Array(10).fill(null).map(() => Array(12).fill(1)),
                        visible: true,
                        opacity: 1.0,
                    }
                ],
                playerSpawns: [{ x: 1, y: 8 }, { x: 2, y: 8 }],
                enemySpawns: [{ x: 10, y: 1 }, { x: 9, y: 1 }],
            },
            playerUnits: [
                {
                    id: 'complex-hero',
                ...
                layers: [
                    {
                        name: 'background',
                        type: 'background',
                        data: Array(4).fill(null).map(() => Array(6).fill(1)),
                        visible: true,
                        opacity: 1.0,
                    }
                ],
...
export class TestDataUtils {
    ...
    static createStageWithUnitCount(playerCount: number, enemyCount: number): StageData {
        ...
        const playerUnits = Array(playerCount).fill(null).map((_, i) => ({
            id: `test-player-${i}`,
            name: `Player ${i + 1}`,
        ...
        const enemyUnits = Array(enemyCount).fill(null).map((_, i) => ({
            id: `test-enemy-${i}`,
            name: `Enemy ${i + 1}`,
    ...
    static generateMemoryTestStage(complexity: 'low' | 'medium' | 'high'): StageData {
        ...
        return {
            ...
            mapData: {
                ...
                tileSize: 32,
                layers: Array(config.layers).fill(null).map((_, i) => ({
                    name: `layer-${i}`,
                    type: i === 0 ? 'background' : 'terrain' as any,
                    data: Array(config.height).fill(null).map(() => Array(config.width).fill(i)),
                    visible: true,
                    opacity: 1.0,
                })),
                playerSpawns: Array(config.players).fill(null).map((_, i) => ({
                    x: 1 + (i % 5),
                    y: config.height - 3 + Math.floor(i / 5)
                })),
                enemySpawns: Array(config.enemies).fill(null).map((_, i) => ({
                    x: config.width - 5 + (i % 5),
                    y: 1 + Math.floor(i / 5)
                })),
            },
            playerUnits: TestDataUtils.createStageWithUnitCount(config.players, config.enemies).playerUnits,
