...
describe('RecruitmentSystem アクセシビリティテスト', () => {
  ...
  describe('キーボード操作のアクセシビリティ', () => {
    ...
    test('Enterキーによる仲間化条件の詳細表示', () => {
      ...
      expect(announcements).toContain('[assertive] 仲間化条件を表示します');
      expect(announcements).toContain('[polite] 条件1: 主人公で攻撃して撃破する - 未達成');
      expect(announcements).toContain('[polite] 条件2: HPが30%以下の状態で撃破する - 未達成');
    });
    ...
    test('Escapeキーによる仲間化UI の閉じる', () => {
      ...
      expect(accessibilityAPI.getAnnouncements()).toContain('[polite] 仲間化条件表示を閉じました');
    ...
    test('スペースキーによる仲間化進捗の音声読み上げ', () => {
      ...
      const announcements = accessibilityAPI.getAnnouncements();
      expect(announcements).toContain('[assertive] 仲間化進捗: 2つの条件のうち1つが達成済みです');
      expect(announcements).toContain('[polite] 達成済み: HPが30%以下の状態で撃破する');
      expect(announcements).toContain('[polite] 未達成: 主人公で攻撃して撃破する');
    });
  ...
  describe('スクリーンリーダー対応', () => {
    test('仲間化対象の詳細情報読み上げ', () => {
      ...
      // 基本情報の読み上げ
      expect(announcements).toContain('[polite] 仲間化可能な敵: Recruitable Knight');
      expect(announcements).toContain('[polite] 位置: 6行6列');
      expect(announcements).toContain('[polite] HP: 80/80, MP: 30/30');
      expect(announcements).toContain('[polite] 仲間化条件: 2つ');
    });
    ...
    test('仲間化条件の詳細読み上げ', () => {
      ...
      expect(announcements).toContain('[assertive] 仲間化条件の詳細を読み上げます');
    ...
    test('仲間化進捗の変化通知', () => {
      ...
      const announcements = accessibilityAPI.getAnnouncements();
      expect(announcements).toContain('[assertive] 仲間化条件が更新されました');
      expect(announcements).toContain('[polite] HPが30%以下の状態で撃破する - 達成しました');
      expect(announcements).toContain('[polite] 仲間化進捗: 50%完了');
    });
    ...
    test('NPC状態変化の通知', () => {
      ...
      expect(announcements).toContain('[polite] ステージクリア時に生存していれば仲間になります');
    });
    ...
    test('仲間化成功・失敗の通知', () => {
      ...
      expect(announcements).toContain('[assertive] 仲間化に成功しました');
      expect(announcements).toContain('[polite] Recruitable Knight が仲間に加わりました');
      ...
      expect(announcements).toContain('[assertive] 仲間化に失敗しました');
      expect(announcements).toContain('[polite] 理由: NPCが撃破されました');
    });
    ...
    test('操作ヘルプの読み上げ', () => {
      ...
      const announcements = accessibilityAPI.getAnnouncements();
      expect(announcements).toContain('[assertive] 仲間化システムの操作方法を説明します');
      expect(announcements).toContain('[polite] Tabキー: 仲間化対象の選択');
      expect(announcements).toContain('[polite] Enterキー: 仲間化条件の詳細表示');
      expect(announcements).toContain('[polite] スペースキー: 現在の進捗状況を読み上げ');
      expect(announcements).toContain('[polite] Escapeキー: 仲間化UI を閉じる');
      expect(announcements).toContain('[polite] 数字キー1-9: 仲間化対象の直接選択');
    });
  });
  ...
  describe('認知的アクセシビリティ', () => {
    test('複雑な条件の簡潔な説明', () => {
      ...
      const announcements = accessibilityAPI.getAnnouncements();
      expect(announcements).toContain('[polite] 仲間化するには3つの条件を満たす必要があります');
      expect(announcements).toContain('[polite] 1. 主人公で攻撃する');
      expect(announcements).toContain('[polite] 2. HPを少なくする（25%以下）');
      expect(announcements).toContain('[polite] 3. 早めに倒す（10ターン以内）');
    });
      ...
      progressStages.forEach((stage, index) => {
        ...
        const progress = {
          ...
          completionPercentage:
            (stage.conditionsMet.filter(met => met).length / stage.conditionsMet.length) * 100,
        };

        recruitmentUI.announceProgressUpdate(target, progress);

        const announcements = accessibilityAPI.getAnnouncements();
        expect(announcements).toContain(`[polite] 仲間化の進捗: ${stage.description}`);
      });
    });
    ...
    test('エラー状況の明確な説明', () => {
      ...
      const announcements = accessibilityAPI.getAnnouncements();
      expect(announcements).toContain('[assertive] 仲間化に失敗しました');
      expect(announcements).toContain('[polite] 理由: 攻撃者が条件を満たしていません');
      expect(announcements).toContain('[polite] 必要な攻撃者: Hero');
      expect(announcements).toContain('[polite] 実際の攻撃者: Wrong Attacker');
    });
    ...
    test('次のアクションの提案', () => {
      ...
      const announcements = accessibilityAPI.getAnnouncements();
      expect(announcements).toContain('[polite] 次のアクション提案');
      expect(announcements).toContain('[polite] Hero で攻撃して撃破してください');
      expect(announcements).toContain('[polite] HP条件は既に満たしています');
    });
  });
  ...
  describe('カスタマイズ可能なアクセシビリティ設定', () => {
    ...
    test('詳細レベルの調整', () => {
      ...
      // 最小限の情報のみ読み上げ
      expect(announcements).toContain('[polite] Recruitable Knight 選択');
