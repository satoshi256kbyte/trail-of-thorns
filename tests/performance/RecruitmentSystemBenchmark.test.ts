        ...
        return {
            ...
            mapData: {
                ...
                tiles: Array(50).fill(null).map(() =>
                    Array(50).fill({ type: 'grass', movementCost: 1 })
                )
            },
            playerUnits,
            enemyUnits,
...
describe('RecruitmentSystem パフォーマンスベンチマーク', () => {
    ...
    describe('条件チェックパフォーマンス', () => {
        ...
        test('並行条件チェックのパフォーマンス', async () => {
            ...
            console.log(`Parallel condition checks (${targets.length} targets): ${duration.toFixed(2)}ms`);
        });
    });
    ...
    describe('NPC状態管理パフォーマンス', () => {
        test('大量NPC変換のパフォーマンス', () => {
            ...
            console.log(`Mass NPC conversion (${recruitableUnits.length} units): ${duration.toFixed(2)}ms`);
        });
        ...
        test('NPC優先度計算のパフォーマンス', () => {
            ...
            // 全ユニットの優先度を計算
            const priorities = allUnits.map(unit =>
                recruitmentSystem.getNPCPriority(unit)
            );
    ...
    describe('UI更新パフォーマンス', () => {
        test('条件表示更新のパフォーマンス', () => {
            ...
            console.log(`UI condition updates (${recruitableUnits.length} units): ${duration.toFixed(2)}ms`);
        });
    ...
    describe('統合パフォーマンス', () => {
        test('完全な仲間化フローのパフォーマンス', () => {
            ...
            console.log(`Complete recruitment flow (${recruitableUnits.length} candidates): ${duration.toFixed(2)}ms`);
        });
        ...
        test('バージョン間のパフォーマンス比較', () => {
            ...
            testCases.forEach(testCase => {
                ...
                if (duration > baseline * 0.8) {
                    console.warn(`Performance warning: ${testCase.name} test took ${duration.toFixed(2)}ms (baseline: ${baseline}ms)`);
                }

                console.log(`${testCase.name} performance: ${duration.toFixed(2)}ms (baseline: ${baseline}ms)`);
            });
        });
    });
});