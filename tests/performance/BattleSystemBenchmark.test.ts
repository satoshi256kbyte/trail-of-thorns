...
describe('Battle System - Performance Benchmark Tests', () => {
  ...
  describe('計算処理の最適化', () => {
    test('攻撃範囲計算のパフォーマンス', async () => {
      ...
      const largeMap = {
        ...
        tiles: Array(200)
          .fill(null)
          .map(() => Array(200).fill({ type: 'grass', movementCost: 1 })),
        obstacles: [],
      };
    ...
    test('パスファインディングのパフォーマンス', async () => {
      ...
      const complexMap = {
        ...
        tiles: Array(100)
          .fill(null)
          .map(() => Array(100).fill({ type: 'grass', movementCost: 1 })),
        obstacles: [],
      };
  ...
  describe('メモリ使用量の監視', () => {
    ...
    test('アニメーションオブジェクトのメモリ管理', async () => {
      ...
      // 同時に複数のアニメーションを実行
      const animationPromises = units.map(unit => animator.showDamageEffect(unit, 25, 'physical'));
