...
const createComprehensiveStageData = (): StageData => ({
  ...
  mapData: {
    ...
    tiles: Array(20)
      .fill(null)
      .map(() => Array(20).fill({ type: 'grass', movementCost: 1 })),
  },
  playerUnits: [
    {
      id: 'protagonist',
...
describe('仲間化システム 要件カバレッジテスト', () => {
  ...
  describe('要件2: 仲間化条件の設定と判定 - 完全カバレッジ', () => {
    ...
    test('2.5 仲間化条件満たして撃破時の成功判定', () => {
      ...
      coverageValidator.markRequirementCovered(
        '2.5',
        'successful recruitment on condition fulfillment'
      );
    });
  });
  ...
  describe('要件5-8: 残りの要件カバレッジ', () => {
    test('要件5: UI表示の完全カバレッジ', () => {
      ...
      recruitmentSystem.checkRecruitmentEligibility(
        characterManager.getUnitById('protagonist'),
        knight
      );
